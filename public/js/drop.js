...
function modalDrop() {
    var html = '<div style="max-width:100% ; max-height: 80vh" class="container">' +
        ...
        '</div>' +
        '</div>' +
        '<label style="margin: -1% auto; min-width: 60%; color: #f6b504 !important; cursor:pointer" class="label"><a href="https://api-smartcomerci.com.br:9090/modelo.xlsx">Download planilha sem produtos</a></label>' +
        '</div>' +
        '<div   class="input-group btnDrop">' +
        '<div style="border: none; margin: auto;" class="input-group-append">' +
        '<div style="border: none; margin: auto; padding: 1%;" class="input-group-text">' +
        ...
        onShow: function () {
            ...
            $("#minhaPlanilha").change(function () {
                ...
                try {
                    ...
                    $.ajax({
                        url: 'https://api-smartcomerci.com.br:9090/uploadPlanilha',
                        headers: {
                        ...
                        success: function (data) {
                            ...
                            $.ajax({
                                url: 'https://api-smartcomerci.com.br:9090/excel2Json',
                                headers: {
                                    "x-access-token": localStorage.token,
                                    "master_id": localStorage.MASTER_ID,
                                    "affiliate_id": localStorage.MASTER_ID,
                                           ...
                                           if(retorno.dados.length > 0){
                                               ...
                                               for(const i in tipos){
                                                ...
                                                for(const k in confere){
                                                    ...
                                                    if(tipos[i].toLocaleLowerCase() == k.toLocaleLowerCase() || k.toLocaleLowerCase() == 'product_thumbail'){
                                                      
                                                        listaConferida.push({"tipo": k.toLocaleLowerCase(), "status": true})       
                                                        isReady = true       
                                                                            
                                                    }
                                                }
                                                if(!isReady){
                                                    listaConferida.push({"tipo": tipos[i].toLocaleLowerCase(), "status": isReady}) 
                                                }
                                               }
                                               for(const k in listaConferida){
                                                   if( !listaConferida[k].status ){ return {"status":false, "msg": listaConferida};}
                                               }
                                    ...
                                    if(verifyTypes().status){
                                        ...
                                        for(const k in listData){
                                                ...
                                                for(const a in listData[k]){
                                                        ...
                                                        if(Number(listData[k][a]) && a.toLocaleLowerCase() != 'product_site_ean'){
                                                            sql += ' '+a+' = '+listData[k][a]+', '
                                                        }else{
                                                            sql += ' '+a+' = "'+listData[k][a]+'", '
                                                        }
                                                ...
                                                sql += ' where product_affiliate_id ='+localStorage.AFFILIATE_ID+' and product_site_ean = '+listData[k].product_site_ean
...
function modalDropConfirma(fileName, totalRowsAffecteds, listOffQuerys) {
    var html = '<div style="max-width:100% ; max-height: 80vh" class="container">' +
        ...
        '<h3 class="arraste">'+fileName+'</h3>' +
   
        '<input style="display:none" type="file" id="minhaPlanilha" />' +

        '<p class="dark4">A planilha selecionada irá alterar <span class="golden4">'+totalRowsAffecteds+' produtos</span></p>' +
        '</div>' +
...
function finalizaAjustes(listOffQuerys){
    $.ajax({
        type: 'POST',
        url: 'https://api-smartcomerci.com.br:9090/multiQuerys',
