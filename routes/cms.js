...
const fs = require('fs');
        try{
                ...
                const form = new formidable.IncomingForm();
                        ...
                        form.parse(req, (err, fields, files) => { 
                            ...
                            const newpath = path.join("", './public/assets/icons/', "cliente_"+files.fileimagem.name);
            ...
            console.log(error)
            res.status(500).json({ message: "Invalid data parameters!", yourData: "error here out", errorMessage: error });
        }
    ...
    app.get('/loja/:loja',  async function  (req, res){
        setup(newAPP)
        res.render("cms_market", {loja:req.params.loja.replace(/_/g," "),MY_DOMAIN: myDomain});     
    });
    ...
    app.get('/editar_home_cores',  async function  (req, res){
     ...
     app.get('/paginas_institucionais/:nome',  async function  (req, res){
        setup(newAPP)
        res.render("cms_paginas_institucionais",  {MY_DOMAIN: myDomain, NOME: req.params.nome});     
    });
    ...
    app.get('/receita_interna/:id',  async function  (req, res){
        setup(newAPP)
        res.render("cms_receita_interna",  {MY_DOMAIN: myDomain, RECEITA: req.params.id});             
    });
    app.get('/cliente/:id',  async function  (req, res){
        setup(newAPP)
      
         conn.query( 'select * from orders where order_client_id = ' +req.params.id, async function (error, orders, fields) {
            if (!error) {
            

              conn.query( 'select * from users_clients where  id = ' +req.params.id, async function (error, me, fields) {
                if (!error) {
                   
                    let ME = JSON.parse(JSON.stringify(me)), ORDERS = JSON.parse(JSON.stringify(orders))
