...
async function startSession() {
  ...
  $("#textoPrivacidade1").val(
    result[0].master_custom_texto_privacidade
    )

  $("#minhaTagManager").val(result[0].master_tag_manager)
  try{
    $("#switch-shadow185")[0].checked = result[0].master_estoque_negativo == 'on' ? true : false
  }catch(err){
...
function getLojasCad(){
    ...
    let html = "", bdDourada = 'bdDourada'
    for(const k in lojas){
        if(lojas[k].id == Number(localStorage.AFFILIATE_ID)){
            bdDourada = 'bdDourada bdDourada2'
        }
         html += '<div class="col-md-12 '+bdDourada+'" style=" margin: auto; ">'+
                    '<div class="row">'+
                        '<div class="col-md-8">'+
                            '<p class="afiliadaName nome">'+lojas[k].affiliates_business_name+'</p>'+
                        '</div>'+
                        '<div class="col-md-2">'+
                            '<button onclick="showFrame(\'deleteLoja\','+lojas[k].id+')" style="display: inline-flex;width: 190px;gap: 8px;" type="button"'+
                        ...
                        '</div>'+
                        '<div class="col-md-2">'+
                            '<button  onclick="editarLoja('+lojas[k].id+')" type="button" class="btn btn-access rounded-pill">Editar</button>'+
                        '</div>'+
                    ...
                    '<div class="row">'+
                        '<div class="col-md-4">'+
                            ...
                            '<p class="contText nomeSocial">'+lojas[k].affiliates_business_name+'</p>'+
                        '</div>'+
                        '<div class="col-md-2">'+
                            '<p class="minTitle">CNPJ</p>'+
                            '<p class="contText cnpj">'+lojas[k].affiliates_business_cnpj+'</p>'+
                        '</div>'+
                        '<div class="col-md-2">'+
                            '<p class="minTitle">Telefone</p>'+
                            '<p class="contText telefone">'+lojas[k].affiliates_business_telefone+'</p>'+
                        '</div>'+
                        '<div class="col-md-4">'+
                            '<p class="minTitle">e-mail</p>'+
                            '<p class="contText email">'+lojas[k].affiliates_business_mail+'</p>'+
                        '</div>'+
                    ...
                    '<div class="row">'+
                        '<div class="col-md-4">'+
                            ...
                            '<p class="contText horario">'+lojas[k].affiliates_business_horario+
                        '</div>'+
                        '<div class="col-md-4">'+
                            '<p class="minTitle">Endereço</p>'+
                            '<p class="contText endereco">'+lojas[k].affiliates_business_endereco+
                        '</div>'+
                        '<div class="col-md-4">'+
                        '</div>'+
                    '</div>'+
...
async function updateAffiliate(dados){
    ...
    let params ={
        ...
        fields :[
            ...
            {column: "affiliates_business_name",value: dados.affiliates_business_name},
            {column: "affiliates_business_telefone",value: dados.affiliates_business_telefone},
            {column: "affiliates_business_mail",value: dados.affiliates_business_mail},
            {column: "affiliates_business_horario",value: dados.affiliates_business_horario},
            {column: "affiliates_business_endereco",value: dados.affiliates_business_endereco},
            {column: "affiliates_business_numero",value: dados.affiliates_business_numero},
            {column: "affiliates_business_cep",value: dados.affiliates_business_cep},
            {column: "affiliates_business_bairro",value: dados.affiliates_business_bairro},
            {column: "affiliates_business_cidade",value: dados.affiliates_business_cidade},
            {column: "affiliates_business_estado",value: dados.affiliates_business_estado},
            {column: "affiliates_business_lat_lon",value: dados.affiliates_business_lat_lon},
            {column: "affiliates_business_tipo",value: dados.affiliates_business_tipo},
            {column: "affiliates_business_cnpj",value: dados.affiliates_business_cnpj},
            {column: "affiliates_business_mapa",value: dados.affiliates_business_mapa},
            {column: "affiliates_business_photo",value: dados.affiliates_business_photo},
        ]
    }
    //console.log(params)
...
async function getEmails(){
 const myEmails = await ajax(mainHost+'/getSingleTable',{
    idName: "master_id",
    idValue: localStorage.MASTER_ID,
