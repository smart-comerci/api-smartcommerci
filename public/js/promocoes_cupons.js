$(document).ready(function () {
  ...
  if (url7.indexOf("promo") > -1 || url7.indexOf("cupo") > -1) {
    ...
    function removePROMOCAO(element) {
      ...
      if (txt) {
        if (
          Array.isArray(OBJETO_MODEL.applicability.especificCategoriesValue)
        ) {
          const findIndex =
    ...
    function removePROMOCAOTAG(element) {
      ...
      if (txt) {
        if (Array.isArray(OBJETO_MODEL.applicability.especificTagsValue)) {
          const findIndex =
            OBJETO_MODEL.applicability.especificTagsValue.findIndex(
              (object) => {
                return object === txt;
              }
            );
    ...
    function subTagInputTAG(ele, elemento, texto) {
      ...
      if (texto) {
        if (Array.isArray(OBJETO_MODEL.applicability.especificTagsValue)) {
          ...
          let lista = [
            ...new Set(OBJETO_MODEL.applicability.especificTagsValue),
          ];
                ...
                if (b === d) {
                  ...
                  if (Array.isArray(myObj[a][b]) && Array.isArray(OBJ[c][d])) {
                    ...
                    if (myObj[a][b] !== OBJ[c][d]) {
                      result += `<b>${b.replace(/\//g, "")}</b> mudou de: <s>"${
                        myObj[a][b]
                      }"</s> para: <span class="log_bold">"${
                        OBJ[c][d]
                      }"</span><br><hr><br>`;
                    }
                  }
