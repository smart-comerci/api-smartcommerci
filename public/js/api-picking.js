...
function editarProduto(elemento){
    ...
    if(dadosProduto.product_average_weight_value > 0){
        $(".editarProdutoModal").find(".pesoProduto").html("aproximadamente "+dadosProduto.product_average_weight_value+''+dadosProduto.product_average_weight_type)
    }else{
        $(".editarProdutoModal").find(".pesoProduto").hide()
    }
    var imagen = dadosProduto.product_thumbnail
    if(imagen == null){
        imagen = 'https://api-smartcomerci.com.br/images/default/produto-sem-imagem.jpg'
    }
    $(".editarProdutoModal").find(".imagenProduto").css("background","transparent url("+imagen+") 0% 0% no-repeat padding-box")
    $(".editarProdutoModal").find(".quantidadePedido").val(Number(dadosProduto.quantidade))
    $(".editarProdutoModal").find(".btnRemoverProdutoModal").attr("product_code",dadosProduto.product_code )
    $(".editarProdutoModal").find(".btnAplicarModal").attr("product_code",dadosProduto.product_code )
    $(".editarProdutoModal").find(".btnDesfazerProdutoModal").attr("product_code",dadosProduto.product_code )
...
function  modalAdicionarProduto(elemento) {
    ...
    if(dadosProduto.product_average_weight_value > 0){
        $(".adicionarProdutoModal").find(".pesoProduto").html("aproximadamente "+dadosProduto.product_average_weight_value+''+dadosProduto.product_average_weight_type)
    }else{
    ...
    $(".adicionarProdutoModal").find(".imagenProduto").css("background","transparent url("+imagen+") 0% 0% no-repeat padding-box")
    $(".adicionarProdutoModal").find(".quantidadePedido").val(Number(dadosProduto.quantidade))
...
async function cancelaProduto(elemento) {
        ...
        elemento.parent().parent().parent().find(".areaProdutoPedido").addClass("removido")
        elemento.parent().parent().parent().find(".cardProduto").addClass("removido")
        elemento.parent().parent().parent().find(".cardProduto").css("border-left","5px solid silver")
        elemento.attr("status","removido")
        elemento.parent().parent().parent().find(".opaco").css("filter","grayscale(1)") 
        elemento.parent().parent().parent().find(".txtEditarProduto").html("Produto removido")
        elemento.parent().parent().parent().find(".txtColetarProduto").html("Voltar produto")
        elemento.parent().parent().parent().find(".txtColetarProduto").parent().attr("status", "removido")
        elemento.parent().parent().parent().find(".iconeColetar").attr("src","/assets/icons/desfazBranco.svg")

        await updateOrderDetails(atualPedido, Number(localStorage.numeroPedido),valorTotal)
...
async function reativaColetaProduto(elemento) {
    ...
    if(elemento.attr("status") == undefined){
        ...
        if(localStorage.listaSeparados != null && localStorage.listaSeparados != undefined && localStorage.listaSeparados != ''){
         ...
         var pedidosFull = JSON.parse(localStorage.PEDIDO_FULL.trim().replace(/\n/g, "").replace(/\r/g, "").replace(/\t/g, "")), listaOk = []
            for(const k in pedidosFull){
                ...
                if(ok){listaOk.push(pedidosFull[k])}
            }
       ...
       console.log(toDo)
        ...
        elemento.parent().parent().parent().parent().find(".cardProduto").css("border-left","5px solid #f6b504")
        ...
        for(const k in atualPedido){
            if(Number(atualPedido[k].product_code) === PRD_CODE){
                atualPedido[k]['comentario'] = atualPedido[k]['comentario'] ? atualPedido[k]['comentario'].replace(/#CANCELADO#/g,'') : ''
            }
        }
        localStorage.ORDER_EDIT = JSON.stringify(atualPedido)
        await updateOrderDetails(atualPedido, Number(localStorage.numeroPedido),valorTotal)
        ...
        elemento.parent().parent().find(".txtColetarProduto").html("Coletar produto")
        elemento.parent().parent().find(".txtColetarProduto").parent().removeAttr("status")
        elemento.parent().parent().parent().find(".iconeColetar").attr("src","/assets/icons/cart.svg") 
    }
...
async function registraFim(elemento) {
    await insertTracing('finalizado', localStorage.numeroPedido,localStorage.listaSeparados)
...
async function adicionarProdutoAoPedido(elemento){
    ...
    var quantidade = elemento.parent().parent().parent().parent().find(".novaQuantidade").val()
  
    var comentario = elemento.parent().parent().parent().parent().find(".textoComentario").val()

    let produtoFinal = FULL_PRICES(detalhesProduto)

    var detalhes = {  
        ...detalhesProduto,
        valor: Number(quantidade) >= produtoFinal.venda.minimo_para_desconto ? produtoFinal.venda.preco_venda : produtoFinal.venda.valor_bruto, 
        caracteristica: "",
...
async function removerProdutoAoPedido(elemento){
    ...
    for(const k in atualPedido){
      ...
      var valorTotal = Number($("#totalPedido").text().replace("R$","").replace(",",".")) 
   var outroValor = 0
...
async function editarProdutoDoPedido(elemento){
    var prd_code = Number(elemento.attr("product_code"))
    var detalhesProduto = getProductSearchInfo(prd_code)
    var quantidade = elemento.parent().parent().parent().parent().find(".quantidadePedido").val()
  
    var comentario = elemento.parent().parent().parent().parent().find("textarea").val()
    ...
    var loader = setInterval(()=>{
        if(pedidos.length > 0 && clientes.length > 0){
            ...
            for(const k in pedidos){
                var html  = '<div  pedido="'+pedidos[k].id+'" onclick="redirect(\'picking-pedido\','+pedidos[k].id+')" class="cardPedido '+getStatusTracing(pedidos[k].id)+'">'+
                                '<div class="topoPedido">'+
                                    '<p class="numeroPedido">#'+pedidos[k].id+'</p> <p class="nomeCliente">'+getClientInfo(pedidos[k].order_client_id).users_client_name+'</p>'+
                                '</div>'+
                                '<div class="areaData">'+
                                    // eslint-disable-next-line no-undef
                                    '<p class="dataRecebimento">* Recebido '+moment(pedidos[k].createdAt).format("DD/MM HH:mm")+'</p>'+
                                '</div>'+
                                '<div class="areaDetalhe">'+
                                    // eslint-disable-next-line no-undef
                                    '<p class="txtDetalhe">'+pedidos[k].order_metodo_entrega+' '+moment(pedidos[k].order_data_entrega).format("DD/MM")+' 09h as 13h - R$ '+Number(pedidos[k].order_valor_entrega).toFixed(2).replace(".",",")+'</p>'+
                                '</div>'+
                            '</div>';
...
async function getTracing(){
    return await  request('getById', {"table": "tracingOrder", "id_name":"affiliate_id", "id_value": AFFILIATE_ID})   
   
}
async function getClients(clientes){
    var dados = await  request('getById', {"table": "users_clients", "id_name":"users_client_affiliate_id", "id_value": AFFILIATE_ID})   
    clientes.push(dados)
}
...
async function search(element){
    ...
    if(localStorage.DADOS_SEARCH != undefined){
     ...
     for(const k in dados){
        ...
        let valorFinal = produtoFinal.existe_desconto === true ? produtoFinal.venda.preco_venda : produtoFinal.venda.valor_bruto
         var peso =    '<p class="pesoModalBusca">aproximadamente '+dados[k].product_average_weight_value+''+dados[k].product_average_weight_type+'</p>'
         if(dados[k].product_average_weight_type == null || dados[k].product_average_weight_type == '' || dados[k].product_average_weight_value == 0 ){peso = ''}
         var html = '<div class="areaBuscaProduto">'+
                        '<img class="imagenCardBusca" src="'+imagen+'" />'+
                        '<div class="areaTextoModal">'+
                            '<p class="descricaoModalBusca">'+dados[k].product_site_name+'</p>'+
                         
                            '<p class="precooModalBusca">R$'+valorFinal.toFixed(2).replace('.',',')+'</p>'+
                        '</div>'+
                        '<div class="right">'+
                            '<div product_code="'+dados[k].product_code+'" onclick="modalAdicionarProduto($(this))" class="areaIconeBuscaModal">'+
