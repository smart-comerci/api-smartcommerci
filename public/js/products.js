...
function removeFiltrosAvancadosLoad() {
    ...
    let totalFiltros = 0, currFilter = null, listFiedlsSearch = ""
    for (const k in parametros) {
        if (parametros[k].active == true) {
            if (currFilter != parametros[k].colmun + "," + parametros[k].active) {
                totalFiltros++
                listFiedlsSearch += parametros[k].colmun + ": " + (parametros[k].value == null ? '' : parametros[k].value) + " ,"
            }
        }
...
function setPARAMETROS_FILTRO(elemento) {
    ...
    if (elemento.attr("id") == "searchProducts") {
        var column = elemento.attr("column"), value = elemento.val()
        let active = false
        if (value != "") {
            active = true
        }
        ...
        var column = elemento.attr("column"), value = elemento.attr("value"), active = elemento[0].checked
...
$("#searchProducts").change(function () {
    ...
    $(".listaDeProdutos").html("")
    request("productSearch", 0, 25, $(this).val(), localStorage.AFFILIATE_ID, $(this).val())
});
...
async function atualizaListaProdutos() {
    ...
    if (localStorage.LAST_REQUEST == undefined || localStorage.LAST_REQUEST == "") {
        var parametros = []
        try {
            parametros = JSON.parse(localStorage.PARAMETROS_FILTROS)
        } catch (e) { }
        if (parametros.length > 0) {
            await buscaPeloFiltro(parametros, 10000, false, 10000, last_request.LAST_ID, true)
        }

    } else {
        PRODUCTS = []
        var last_request = JSON.parse(localStorage.LAST_REQUEST)
        request(last_request.ADDRESS, last_request.LAST_ID, last_request.TOTAL_ITENS, last_request.PRODUCT_CODE, last_request.PRODUCT_AFFILIATE_ID, last_request.PRODUCT_NAME)
...
async function request(ADDRESS, LAST_ID, TOTAL_ITENS, PRODUCT_CODE, PRODUCT_AFFILIATE_ID, PRODUCT_NAME) {
    ...
    $.ajax({
        ...
        success: function (products) {
            ...
            if (products.length == 0) {
                var myText = 'para "<b style="color:#f6b504">' + PRODUCT_NAME + '</b>" na'
                if (PRODUCT_NAME == 0) {
                    myText = ' na'
                }
                var semRetorno = '<div style="max-width: 60%; margin-top: 50px" class="container">' +
                    //  '<p class="textoSemResultado">Nenhum resultado para "<b style="color:#f6b504">'++'</b>"</p>'+
                    '<p class="textoSemResultado">Nenhum resultado ' + myText + ' unidade  ' + $("#filialShow").text().replace('/', '') + '</p>' +
                    '<br><br>' +
                    '<p class="oqueFazer">O que posso fazer?</p>' +

                    '<p class="listaFazer"><div class="arrowCheck">' + arrowCheck1 + '</div> Verifique os termos digitados</p>' +
                    '<p class="listaFazer"><div class="arrowCheck">' + arrowCheck1 + '</div> Tente usar uma única palavra</p>' +
                    '</div>'
            ...
            for (const k in products) {
                ...
                if (pictureToShow == null || pictureToShow == undefined || pictureToShow == 'null' || pictureToShow == '') {
                ...
                var ativo = ''
                if (getProductCaract(products[k].product_code).product_status == 'active') {
                    ativo = 'checked="true"'
                }
                ...
                var html =
                    '<div onclick="showModalProductF($(this))" product_thumbnail="' + products[k].product_thumbnail + '" product_code="' + products[k].product_code + '" product_ean="' + products[k].product_ean + '"  affiliate_id="' + products[k].product_affiliate_id + '" class="row radius20 product ' + especialClass + '" id="element' + k + '_' + products[k].product_ean + '">' +
                    ...
                    '<div style="background: url(' + pictureToShow + '), url(https://api-smartcomerci.com.br/images/default/produto-sem-imagem.jpg)" class="image img">' +
                    ...
                    '<input  onchange="acaoCheckbox($(this))" acao="ativarProduto" ' + ativo + ' product_code="' + products[k].product_code + '" id="switch-shadow' +
                    (k + 30) +
                ...
                count++;
                getProductData(products[k].product_ean, '#element' + k + '_' + products[k].product_ean, $('#element' + k + '_' + products[k].product_ean))
            }
...
async function personalRequest(ADDRESS, LAST_ID, TOTAL_ITENS, order_type, column_order, PRODUCT_AFFILIATE_ID, continua) {
    ...
    $.ajax({
        ...
        success: function (products) {
            ...
            if (products.length == 0) {
                ...
                for (const k in lojas2) {
                    if (lojas2[k].id != Number(localStorage.AFFILIATE_ID)) {
                        $(".listaFiliaisC").append("<button onclick='setLocal(" + lojas2[k].id + ")' class='btn btn-primary'>Ver <b>'" + lojas2[k].affiliates_business_name + "'</b></button>")
                    }
                    if (lojas2[k].id == Number(localStorage.AFFILIATE_ID)) {
            ...
            for (const k in products) {
                ...
                var html =
                    '<div onclick="showModalProductF($(this))" product_code="' + products[k].product_code + '" product_ean="' + products[k].product_ean + '"  affiliate_id="' + products[k].product_affiliate_id + '" class="row radius20 product ' + especialClass + '" id="element' + k + '_' + products[k].product_ean + '">' +
                    '<div style="max-width: 70px;" class="col-sm">' +
                    '<label class="checkSmart"><input class="checka form-control" type="checkbox" /><span class="checkmark"></span></label>' +
                    "</div>" +
                    '<div  class="col-sm imgContainer">' +
                    ...
                    '<div class="col-sm">' +
                    '<div class="switch__container  checkaLiga">' +
                    '<input acao="ativarProduto" ' + ativo + ' product_code="' + products[k].product_code + '" id="switch-shadow' +
                    (k + 30) +
                    '" class="switch switch--shadow" type="checkbox" />' +
                    '<label class="naoAbreModal" for="switch-shadow' +
                    (k + 30) +
...
function modalProduct(PRODUCTS, product_code, affiliate_id, URLS) {
    ...
    var product_code, affiliate_id, product_data = [];
    ...
    try {
        ...
        if (product_data[0].product_code == undefined || product_data[0].product_code == "undefined") {
            product_data = [{
                id: null,
                product_affiliate_id: AFFILIATE_ID,
                product_categoria: '',
    ...
    try{
        ...
        if(DESCONTOS_FULL['subtracaoProduto'].active == undefined){ DESCONTOS_FULL['subtracaoProduto'].active = false }
        if(DESCONTOS_FULL['porcentagemProduto'].active == undefined){ DESCONTOS_FULL['porcentagemProduto'].active = false }
        if(DESCONTOS_FULL['subtracao'].active == undefined){ DESCONTOS_FULL['subtracao'].active = false }
    ...
    if (product_data[0].product_site_discount_type == null || product_data[0].product_site_discount_type == 'null') { } else {
        if (product_data[0].product_site_discount_type.indexOf("Promoção preço total") > -1) {
            precoTotalCK = 'checked="true"'
        }
        if (product_data[0].product_site_discount_type.indexOf("Promoção compre X pague Y") > -1) {
            pagueLeveCK = 'checked="true"'
        }
        if (product_data[0].product_site_discount_type.indexOf("Por produto no carrinho") > -1) {
            byCarCK = 'checked="true"'
        }
    }

    if (product_data[0].product_site_related_type == null || product_data[0].product_site_related_type == 'null') { } else {
        if (product_data[0].product_site_related_type.indexOf("Smart") > -1) {
            smart = 'checked="true"'
        }
        if (product_data[0].product_site_related_type.indexOf("mais vendidos") > -1) {
            maisVendidos = 'checked="true"'
        }
        if (product_data[0].product_site_related_type.indexOf("Ofertas") > -1) {
            ofertas = 'checked="true"'
        }
        if (product_data[0].product_site_related_type.indexOf("Personalizado") > -1) {
   ...
   // console.log('DESCONTOS_FULL',DESCONTOS_FULL)
    ...
    if (product_data[0].product_site_nutrition == null || product_data[0].product_site_nutrition == undefined || product_data[0].product_site_nutrition == "") {
    ...
    if (informacaoNutritiva == null) {
        informacaoNutritiva = [{ quantidade: 0, titulo: "" }, { quantidade: 0, titulo: "" }]
    }
    if (product_data[0].product_site_nutrition != null && product_data[0].product_site_nutrition != '') {
        nutricional = 'checked="true"'
    }
    if (
        product_data[0].product_average_weight_value != null && product_data[0].product_average_weight_type != "" &&
        product_data[0].product_average_weight_value != "" && product_data[0].product_average_weight_type != null
    ) {
        vendaPorPesoAutoriza = '<span class="metricas">1 unidade - aproximadamente ' + product_data[0].product_average_weight_value + ' ' + product_data[0].product_average_weight_type + '</span><br>';
    }

    if (product_data[0].product_sell_by_weight != '' &&
    ...
    function confere(valor, tipo) {
        if (valor == null || valor == undefined || valor == 'null' || valor == 'undefined' || valor == '') {
            if (tipo == 'number') { return 0 } else {
                return ''
            }
        } else {
            return valor
    ...
    var htmlModal =
        ...
        imageShows(product_data[0].product_affiliate_id, product_data[0].product_code, URLS[0]) +

        "</div>" +
        '<ul class="preview-thumbnail nav nav-tabs imagensProduto">' +

        //drop here the thumb images
        imageButtons(product_data[0].product_affiliate_id, product_data[0].product_code, URLS) +
        ...
        '<input ' + produtoAtivo + ' acao="ativarProduto"  onchange="acaoCheckbox($(this))" product_code="' + product_code + '" id="switch-shadow6" class="switch switch--shadow" type="checkbox" />' +
        ...
        "</div>" +
        '<div style="padding:0 2%; margin-top: -2%" class="row">' +
        '<div class="group-input2"><input  acao="product_site_name" product_code="' + product_data[0].product_code + '" onChange="updateField($(this))" value="' +
        ...
        "</div>" +
        '<div style="padding:0 1%;     margin-top: -2%;" class="row">' +
        '<div class="col-md-8"> <div class="group-input2" style="display: inline-flex;"><div class="iconLogistica"  style=" height: 45px;width: 50px; margin: auto 2%!important;    background: #F9FAFB 0% 0% no-repeat padding-box !important;">' + money + '</div><input acao="product_valor" product_code="' + product_data[0].product_code + '" onChange="updateField($(this))" class="form-control inputProduct" placeholder="R$ 0.00" value="' +
        product_data[0].product_valor +
        '" id="valorProduto"></div></div> ' +
        '<div class="col-md-4"> <div class="group-input2" style="display: inline-flex;"><input     product_code="' + product_data[0].product_code + '"  acao="product_site_estoque"  onChange="updateField($(this))"  value="' +
        product_data[0].product_valor +
        '" class="form-control inputProduct" placeholder="0.00" id="qtdProduto"><input style="border-left: 1px solid #f6b504" acao="product_average_weight_type" product_code="' + product_data[0].product_code + '" onChange="updateField($(this))" class="form-control inputProduct" placeholder="Un,Kg" value="' +
        ...
        "</div>" +
        '<div style="padding:0 2%; margin-top: -2%;" class="row">' +
        '<div class="group-input2"><input value="' + product_data[0].product_site_ean + '" product_code="' + product_data[0].product_code + '"  acao="product_site_ean"  onChange="updateField($(this))" class="form-control inputProduct" placeholder="789456132012" id="eanProduto"></div><br> ' +
        "</div>" +
        '<hr style="opacity:0">' +
        ...
        "</div>" +
        '<div style="padding:0 2%; margin-top: -2%;" class="row">' +
        '<div class="group-input2"><input  product_code="' + product_data[0].product_code + '"  acao="product_site_fabricacao"  onChange="updateField($(this))" value="' + product_data[0].product_site_fabricacao + '" class="form-control inputProduct" placeholder="Marca" id="marcaProduto"></div><br> ' +
        "</div>" +
        '<hr style="opacity:0">' +

        '<div style="margin-top: 3% !important; ' + (compraPorPeso ? 'border: 2px solid #f6b504' : '') + '" class="col-md-12 grupo">' +

        '<div style="padding:0 2%; margin-top: 2%" class="row">' +

        '<div style="margin: 1% 2%;" class="switch__container  ajuste">' +
        '<input ' + mostrarPeso + ' id="switch-shadow4" objetivo="mostrarPeso" onchange="alteraPeso($(this))" class="switch switch--shadow" type="checkbox" />' +
        ...
        '</div><input acao="product_average_weight_value" product_code="' + product_data[0].product_code + '" onChange="updateField($(this))" type="number" value="' + product_data[0].product_average_weight_value + '" class="form-control inputProduct" style="background: none; border: none" placeholder="0.00" id="nomeProduto"></div></div> ' +
        '<div class="col-md-6"><select  acao="product_average_weight_type" product_code="' + product_data[0].product_code + '" onChange="updateField($(this))" value="' + product_data[0].product_average_weight_type + '" style="border: none; margin:10% auto;text-align: right;float: right;font-size: 1rem" class="form-control"><option>' + product_data[0].product_average_weight_type + '</option><option>gramas</option><option>litros</option><option>centimetros</option></select></div> ' +
        "</div>" +
        "</div>" +

        '<div style="display: ' + feedbackPeso + '; " class="col-md-6 areaMostraPeso">' +
        '<div style="margin: 3% 2%;" class="switch__container  ajuste">' +
        '<input ' + compraPorPeso + '  objetivo="compraPorPeso" onchange="alteraPeso($(this))" id="switch-shadow5" class="switch switch--shadow" type="checkbox" />' +
        ...
        getCategoriesAndSub(MY_CATEGORIES, product_data[0].product_site_categories) +
        ...
        "</div>" +
        '<div class="col-md-12 grupoCinza">' +
        '<textarea   acao="product_site_description" product_code="' + product_data[0].product_code + '" onChange="updateField($(this))"  style="background: #EFEFEF; border:none; font-size: 1.3rem; max-height: 100%" rows="3"   class="form-control">' +
        product_data[0].product_site_description + '</textarea>' +
        "</div>" +
        '<hr style="opacity:0">' +
        '<div style="padding:0 2%" class="row">' +
        ...
        '<div style="padding:0 2%; margin-top: 2%" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container  ajuste">' +
        '<input ' + moreInfo + ' id="switch-shadow7" class="switch switch--shadow" type="checkbox" />' +
        '<label for="switch-shadow7"></label>' +
        "</div>" +
        '<label style=" font-size: 20px;" class="label">Informação Importante:</label> ' +
        "</div>" +
        '<div style="padding:0 2%" class="row">' +
        '<textarea   acao="product_site_info" product_code="' + product_data[0].product_code + '" onChange="updateField($(this))"  style="background: #EFEFEF; border:none; font-size: 1.3rem; max-height: 100%;    padding: 2%;" rows="3" class="form-control">' +
        ...
        '<div style="padding:0 2%; margin-top: 2%" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container  ajuste">' +
        '<input ' + nutricional + ' id="switch-shadow12" class="switch switch--shadow" type="checkbox" />' +
        ...
        '<input  id="switch-shadow13" acao="tipoDesconto" ' + precoTotalCK + ' product_code="' + product_data[0].product_code + '" tipoDesconto="Promoção preço total" class="switch switch--shadow" type="checkbox" />' +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div  style="max-width: 50px;" class="col-md-1">' +
        '<label class="checkSmart checkDireita animate__animated animate__"><input '+(DESCONTOS_FULL.subtracao.active ? 'checked="true"':'')+' onchange="marcaPromocao($(this))" class="marcaPromocao marcaPsubtracao" tipo="subtracao" type="checkbox"><span class="checkmark"></span></label>' +
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2"><div class="iconLogistica"  style=" height: 45px;width: 50px; margin: auto 2%!important;    background: #F9FAFB 0% 0% no-repeat padding-box !important;">' + money + '</div><input value="' + confere(DESCONTOS_FULL.subtracao.valorDescontado, 'number') + '" style="max-width: 65%" class="form-control inputProduct darDesconto" origem="Promoção preço total"  preco="' +
        product_data[0].product_valor +
        ...
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2"  style="background: none"><input value="' + confere(DESCONTOS_FULL.subtracao.precoFinal, 'number') + '"  style="background: none" class="form-control inputProduct valorDescontadoF" placeholder="-" id="nomeProduto"></div><br> ' +
        "</div>" +
        '<div class="col-md-3">' +
        "</div>" +
        "</div>" +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div style="max-width: 50px;" class="col-md-1">' +
        '<label class="checkSmart checkDireita animate__animated animate__"><input  '+(DESCONTOS_FULL.porcentagem.active ? 'checked="true"':'')+'  onchange="marcaPromocao($(this))"  class="marcaPromocao marcaPporcentagem" tipo="porcentagem" type="checkbox"><span class="checkmark"></span></label>' +
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2"><div class="iconLogistica"  style=" height: 45px;width: 50px; margin: auto 2%!important;    background: #F9FAFB 0% 0% no-repeat padding-box !important;">' + porcentagem + '</div><input value="' + confere(DESCONTOS_FULL.porcentagem.percentualDescontado, 'number') + '" style="max-width: 65%" class="form-control inputProduct darDesconto"  origem="Promoção preço total" tipo="porcentagem" preco="' +
        product_data[0].product_valor +
        '" placeholder="-" id="nomeProduto"></div><br> ' +
        //'<div class="group-input2"><input class="form-control inputProduct" placeholder="-" id="nomeProduto"></div><br> '+
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2" style="background: none"><input value="' + confere(DESCONTOS_FULL.porcentagem.precoFinal, 'number') + '"  style="background: none" class="form-control inputProduct valorDescontadoF" placeholder="-" id="nomeProduto"></div><br> ' +
        "</div>" +
        '<div class="col-md-3">' +
        "</div>" +
        "</div>" +
        ...
        '<div style="padding:0 2%; margin-top: 2%" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container  ajuste ativaPromocao">' +
        '<input id="switch-shadow14" acao="tipoDesconto" ' + pagueLeveCK + ' product_code="' + product_data[0].product_code + '" tipoDesconto="Promoção compre X pague Y" class="switch switch--shadow" type="checkbox" />' +
        ...
        product_data[0].product_valor +
        '" tipo="levePague" value="' + confere(DESCONTOS_FULL.levePague.valorDescontado.split(" ")[1], 'number') + '" placeholder="-" id="compreD"></div><br> ' +
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2" ><input class="form-control inputProduct darDesconto"    origem="Promoção compre X pague Y" preco="' +
        product_data[0].product_valor +
        '"  tipo="levePague" value="' + confere(DESCONTOS_FULL.levePague.valorDescontado.split(" ")[3], 'number') + '" placeholder="-" id="pagueD"></div><br> ' +
        "</div>" +
        '<div class="col-md-5">' +
        '<div class="group-input2"  style="background: none"><input  style="background: none" class="form-control inputProduct valorDescontadoD" value="' + confere(DESCONTOS_FULL.levePague.precoFinal, 'number') + '" placeholder="-" id="valorDescontadoD"></div><br> ' +
        "</div>" +
        "</div>" +
        "</div>" +
        '<div style="margin-top: 3% !important" class="col-md-12 grupo">' +
        '<div style="padding:0 2%; margin-top: 2%" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container  ajuste ativaPromocao">' +
        '<input id="switch-shadow15" acao="tipoDesconto" ' + byCarCK + ' product_code="' + product_data[0].product_code + '" tipoDesconto="Por produto no carrinho" class="switch switch--shadow" type="checkbox" />' +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div style="max-width: 50px;" class="col-md-1">' +
        '<label class="checkSmart checkDireita animate__animated animate__"><input  '+(DESCONTOS_FULL.subtracaoProduto.active ? 'checked="true"':'')+'  onchange="marcaPromocao($(this))"   class="marcaPromocao marcaPsubtracaoProduto" tipo="subtracaoProduto" type="checkbox"><span class="checkmark"></span></label>' +
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2"><div class="iconLogistica"  style=" height: 45px;width: 50px; margin: auto 2%!important;    background: #F9FAFB 0% 0% no-repeat padding-box !important;">' + money + '</div><input value="' + confere(DESCONTOS_FULL.subtracaoProduto.valorDescontado, 'number') + '"  style="max-width: 65%" class="form-control inputProduct darDesconto" origem="Desconto por outros produtos no carrinho"  preco="' +
        product_data[0].product_valor +
        '" tipo="subtracaoProduto" placeholder="-" id="nomeProduto"></div><br> ' +
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2" style="background: none"><input value="' + confere(DESCONTOS_FULL.subtracaoProduto.precoFinal, 'number') + '"  style="background: none" class="form-control inputProduct valorDescontadoF" placeholder="-" id="nomeProduto"></div><br> ' +
        "</div>" +
        '<div class="col-md-3">' +
        "</div>" +
        "</div>" +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div style="max-width: 50px;" class="col-md-1">' +
        '<label class="checkSmart checkDireita animate__animated animate__"><input  '+(DESCONTOS_FULL.porcentagemProduto.active ? 'checked="true"':'')+'  onchange="marcaPromocao($(this))"  class="marcaPromocao marcaPporcentagemProduto" tipo="porcentagemProduto" type="checkbox"><span class="checkmark"></span></label>' +
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2"><div class="iconLogistica"  style=" height: 45px;width: 50px; margin: auto 2%!important;    background: #F9FAFB 0% 0% no-repeat padding-box !important;">' + porcentagem + '</div><input value="' + confere(DESCONTOS_FULL.porcentagem.percentualDescontado, 'number') + '" style="max-width: 65%" class="form-control inputProduct darDesconto"  origem="Desconto por outros produtos no carrinho"  preco="' +
        product_data[0].product_valor +
        ...
        "</div>" +
        '<div class="col-md-3">' +
        '<div class="group-input2" style="background: none"><input value="' + confere(DESCONTOS_FULL.porcentagemProduto.precoFinal, 'number') + '"  style="background: none;" class="form-control inputProduct valorDescontadoF" placeholder="-" id="nomeProduto"></div><br> ' +
        "</div>" +
        '<div class="col-md-3">' +
        "</div>" +
        "</div>" +
        ...
        '<div style="margin: 1% 2%;" class="switch__container  ajuste">' +
        '<input ' + relacionadosAtivado + ' id="switch-shadow18" class="switch switch--shadow" type="checkbox" />' +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div class="col-md-12">' +
        '<div class="group-input2"  style="background: #F0F0F0 0% 0% no-repeat padding-box;border: 1px solid #EFEFEF;border-radius: 5px;"><input value="' + product_data[0].product_site_related_title + '"  acao="product_site_related_title" product_code="' + product_data[0].product_code + '" onChange="updateField($(this))"  style="background: none" class="form-control inputProduct" placeholder="Titulo dos relacionados" id="nomeProduto"></div><br> ' +
        "</div>" +
        "</div>" +
        "</div>" +
        '<div style="padding:0 2%" class="row">' +
        ...
        '<input ' + smart + '  acao="relacionados" tipoRelacionamento="Smart" product_code="' + product_data[0].product_code + '" id="switch-shadow' +
        ...
        '<div style="padding:0 2%; margin-top: 2%" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container  ajuste ativaRelacao">' +
        '<input ' + maisVendidos + ' acao="relacionados" product_code="' + product_data[0].product_code + '" tipoRelacionamento="Produtos mais vendidos" id="switch-shadow19" class="switch switch--shadow" type="checkbox" />' +
        ...
        '<div style="padding:0 2%; margin-top: 2%" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container  ajuste ativaRelacao">' +
        '<input ' + ofertas + ' product_code="' + product_data[0].product_code + '"  acao="relacionados" tipoRelacionamento="Ofertas" id="switch-shadow193" class="switch switch--shadow" type="checkbox" />' +
        ...
        '<div style="padding:0 2%; margin-top: 2%" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container  ajuste ativaRelacao">' +
        '<input ' + personal + ' product_code="' + product_data[0].product_code + '"  acao="relacionados" tipoRelacionamento="Personalizados" id="switch-shadow194" class="switch switch--shadow" type="checkbox" />' +
        ...
        '<div class="row">' +
        '<div class="col-md-2">' +
        getS_TAGS(product_data[0].product_site_tags, '<i class="fas fa-heart"></i>') +
        ...
        "</div>" +
        '<div class="col-md-6">' +
        '<img   id="superMostraImg" style="min-width: 80%;max-width:80%; margin:auto" src="' + imagen_URL(product_data[0].product_ean, product_data[0].product_code, PRODUCTS_IMAGES)[0] + '">' +
        "</div>" +
        '<div class="col-md-4">' +
            ...
            onShow: function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $(".listaDeCartoesSelecionados").html(cardSelecionado2(JSON.parse(product_data[0].product_site_related_code)))
                ...
                $(".adicionaTabelaNova").click(function () {
                    var html5 = '<div style="padding:0 2%" class="row removeNovoTabela">' +
                        '<div class="col-md-5 ">' +
                        '<div style="padding:0 2%; margin-top: 2%" class="row">' +
                        '<label style=" font-size: 20px;" class="label">Título</label><br> ' +
                        "</div>" +
                        ...
                        '<div class="col-md-5">' +
                        '<div style="padding:0 2%; margin-top: -2%" class="row">' +
                        '<div class="group-input2"><input  onchange="setNutrition($(this))" name="titulo" class=" tt form-control inputProduct nutriTitulo' + listaNutricionalIndex + ' nutritiones"   placeholder="Título" id="nomeProduto"></div><br> ' +
                        "</div>" +
                        "</div>" +
                        '<div class="col-md-5">' +
                        '<div style="padding:0 2%; margin-top: -2%" class="row">' +
                        '<div class="group-input2"><input  onchange="setNutrition($(this))" name="valor" class=" qt form-control inputProduct nutriQuantidade' + listaNutricionalIndex + ' nutritiones"   placeholder="Quantidade" id="nomeProduto"></div><br> ' +
                        "</div>" +
                        "</div>" +
                        '<div class="col-md-2">' +
                ...
                $(".buscaPRD").keyup(function () {
                    var elemento = $(this).parent().parent().parent().find(".listaDeCartoes")
                    var tag = $(this).parent().parent().parent().parent().find(".listaDeCartoesSelecionados")
                    var texto = $(this).val()
                ...
                $(".darDesconto").change(function () {
                    calculaDesconto($(this).attr("tipo"), $(this), Number($(this).attr("preco")), $(this).attr("origem"));
                });
                ...
                $("#pegaFoto").change(function () {
                    uploadPicture($("#pegaFoto"), product_data[0].product_code, AFFILIATE_ID)
                })
                ...
                function uploadPicture(elemento, product_code, affiliate_id) {
                    ...
                    var urlNew = 'https://api-smartcomerci.com.br:9090/images/' + affiliate_id + '/' + product_code + '/' + elemento[0].files[0].name;
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            //////////console.log(data)
                            var ur = '<li draggable="true" picture_name="' + elemento[0].files[0] + '" class=" thumbProduct ">' +
                                '<figure class="figurefx pushup"><div onclick="deletaImagen(\'secundaria\',\'' + affiliate_id + '\',\'' + product_code + '\',\'' + urlNew + '\',' +
                                '\'' + elemento[0].files[0] + '\', $(this))" class="col-sm paiIcon2">' +
                                '</div><img   id="1_8_2" class="imageThumb notCrash firstImage droptarget" src="' + urlNew + '"><figcaption onmouseleave="mLeave($(this))"  onmouseover="mOver($(this))" class="miuda">' +
                                '<div onclick="deletaImagen(\'secundaria\',\'' + affiliate_id + '\',\'' + product_code + '\',\'' + urlNew + '\',\'' + elemento[0].files[0] + '\', $(this))" class="col-sm paiIcon2"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 21" class="icoPopUpDelete superFica">&gt;<defs></defs><path class="a" d="M10.937,16H3.063A2.208,2.208,0,0,1,.875,13.778V2.666H.438a.444.444,0,0,1,0-.889H4.375V1.334A1.324,1.324,0,0,1,5.687,0H8.313A1.324,1.324,0,0,1,9.625,1.334v.444h3.937a.444.444,0,0,1,0,.889h-.437V13.778A2.208,2.208,0,0,1,10.937,16ZM1.75,2.666V13.778a1.325,1.325,0,0,0,1.313,1.334h7.875a1.325,1.325,0,0,0,1.313-1.334V2.666ZM5.687.889a.441.441,0,0,0-.437.445v.444h3.5V1.334A.441.441,0,0,0,8.313.889Zm3.5,11.556A.442.442,0,0,1,8.75,12V5.778a.437.437,0,1,1,.875,0V12A.441.441,0,0,1,9.188,12.445Zm-4.375,0A.441.441,0,0,1,4.375,12V5.778a.437.437,0,1,1,.875,0V12A.442.442,0,0,1,4.812,12.445Z" transform="translate(4 3)"></path>' +
                            ...
                            var alerta = bootbox.confirm(
                                {
                                    message: 'Imagen salva com sucesso! <img   style="width: 60px; " src="' + urlNew + '" /><br>',
                                    onShow: function () {
                ...
                if (src == 'null' || src == null) {
                    if (imageMain == 'null' || imageMain == null) {
                        $(".preview-pic").find("img").attr("src", "images/default/produto-sem-imagem.jpg")
                    } else {
                ...
                */
                $(".thumbProduct").on("drop", function (event) {
                    ...
                    if (event.target.className.indexOf("droptarget") > -1) {
                        ...
                        imagenPadrao(product_data[0].product_ean, product_data[0].product_code, localStorage.url1)
...
function modalNewProduct() {
    $.ajax({
        ...
        success: async function (data) {
            ...
            $(".listaDeProdutos").html("")
            request("productSearch", 0, 25, data.product_code_new, localStorage.AFFILIATE_ID, data.product_code_new)
...
function getProductInfo(PRODUCTS, product_code, affiliate_id) {
    ...
    var result = [];
    for (const k in PRODUCTS) {
        if (PRODUCTS[k].product_code == product_code && PRODUCTS[k].product_affiliate_id == Number(affiliate_id)) {
...
function calculaDesconto(tipo, elemento, precoProduto, origem) {
    ...
    if (localStorage.listaDescontos.length > 0 && localStorage.listaDescontos != "0" && localStorage.listaDescontos != "null" && localStorage.listaDescontos != null) {
        try {
    ...
    if (tipo == "subtracao") {
        var res = Number((precoProduto - Number(elemento.val().replace(',', '.'))).toFixed(2).replace(",", "."));
        ...
        elemento.parent().parent().parent().find(".valorDescontadoF").css("color", "#f6b504 !important");
        elemento.parent().parent().parent().find(".valorDescontadoF").css("border", "2px solid #f6b504 !important");
    ...
    if (tipo == "porcentagem") {
      //  console.log('calculando aqui ... ')
        ...
        elemento.parent().parent().parent().find(".valorDescontadoF").css("color", "#f6b504 !important");
        elemento.parent().parent().parent().find(".valorDescontadoF").css("border", "2px solid #f6b504 !important");
        var percentual = ((elemento.val() * precoProduto) / 100).toFixed(2).replace(",", ".");
    ...
    if (tipo == "levePague") {
        var res = Number(((Number($("#pagueD").val().replace(',', '.')) * precoProduto) / Number($("#compreD").val()).toFixed(2).replace(",", ".")));
        elemento.parent().parent().parent().find(".valorDescontadoD").val(res);
        elemento.parent().parent().parent().find(".valorDescontadoD").addClass("selecionado2");
        elemento.parent().parent().parent().find(".valorDescontadoD").css("color", "#f6b504 !important");
        elemento.parent().parent().parent().find(".valorDescontadoD").css("border", "2px solid #f6b504 !important");
        listaDescontos.levePague = {
            "origem": origem,
            "tipo": "levePague",
            "precoProduto": precoProduto,
            "valorDescontado": "leve " + $("#compreD").val() + " pague " + $("#pagueD").val(),
            "precoFinal": res
        }
...
function removerei(elemento, desmarcar) {
    ...
    if (elemento.parent().attr("removeStorage") == "relacionados") {
        localStorage.relacionados = localStorage.relacionados.replace(/$(this).parent().text()/g, "")
    }
...
function retornaCategorias(texto) {
    ...
    if (texto != null) {
        if (texto.split(",").length == 0) {
            var html = '<div  class="input-group categoriaLabel">' + "<label  >" + texto + "</label>" + '<label onclick="meDesmarca(\'' + texto + '\')"  class="iconClose"><i class="far fa-times-circle"></i></label>' + "</div>";
            ...
            for (const k in lista) {
                if (lista[k].trim() != "") {
                    var html = '<div  class="input-group categoriaLabel">' + "<label  >" + lista[k] + "</label>" + '<label  onclick="meDesmarca(\'' + lista[k] + '\')"  class="iconClose"><i class="far fa-times-circle"></i></label>' + "</div>";
            ...
            $.ajax({
                ...
                success: function (picture) {
                    ...
                    if (picture.thumbnail != undefined) {
                        var novo = true
                        for (const k in PRODUCTS_IMAGES) {
                            if (EAN == PRODUCTS_IMAGES[k].EAN) { novo = false }
                        }
                        if (novo) {
                    ...
                    try {
                        ...
                        $.ajax({
                            ...
                            success: function (data2) {
                                ...
                                //////console.log(data2)
                                $(elementParent).find(".firstImage").attr("src", mainHost + "/images/" + $(elementParent).attr("affiliate_id") + "/" + $(elementParent).attr("product_code") + "/" + data2[0])
                            },
                            error: function (data2) {
                                //////console.log(data2)
                                try {
                        ...
                        $(elementParent).find(".img").css("background", "url(" + picture.thumbnail + ")")
                        $(elementParent).find(".description").text(picture.description)
                        $(elementParent).find(".fullDescription").attr("src", "THIS FULL DESCRIPTION FROM COSMOS")
                        pai.removeClass("animated-background")
                    }
                ...
                error: function (data) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            //////////console.log("eu achei")
                            //////////console.log(data)
                            $(elementParent).find(".firstImage").attr("src", mainHost + "/images/" + $(elementParent).attr("affiliate_id") + "/" + $(elementParent).attr("product_code") + "/" + data[0])
                        },
                        error: function (data) {
                            //////////console.log(data)
                            if (data.responseJSON.message.indexOf("token") > -1) {
...
function getCategoriesAndSub(MY_CATEGORIES, escolhidas) {
    ...
    for (const k in MY_CATEGORIES) {
        ...
        if (MY_CATEGORIES[k].subCategorias != "?") {
            ...
            for (let a = 0; a < txtCategories.length; a++) {
                if (txtCategories[a].length > 0 && txtCategories[a] != '' && txtCategories[a] != 'null' && txtCategories[a] != 'undefined' && txtCategories[a] != null && txtCategories[a] != undefined) {
                    ...
                    content += '<li   class="list-sub-item targetBusca"><div class="row"><span style="border-top: 5px dotted silver !important;" class="trilha">..........</span><label class="subSmart  animate__animated animate__"><input class="marcar" onchange="changeMarcar($(this),this)" ' + ativoOuNao(txtCategories[a]) + '  ele="miseravi" type="checkbox"><span class="checkmark"></span>' + txtCategories[a] + '</label></div></li> ';
                }
        ...
        content += '</ul>';
        html3 += MY_CATEGORIES[k].categoria + ' <input ' + ativoOuNao(MY_CATEGORIES[k].categoria) + '  class="marcar"  onchange="changeMarcar($(this),this)" type="checkbox"><span class="checkmark subCheck"></span></label>';
        html3 += content + '</li> ';
    }
...
function getTagsAndMarcas(TEXTAO, tipo) {
    ...
    for (const k in LISTA_01) {
        if (LISTA_01[k] != "") {
            ...
            html3 += LISTA_01[k] + ' <input class="marcar"  onchange="subTagInput($(this),\'' + elemento + '\',\'' + LISTA_01[k] + '\')" type="checkbox"><span class="checkmark subCheck"></span></label>';
            html3 += '</li> ';
        }
            ...
            if (k > 0) {
                ...
                if (URLS[k] != undefined) {
                    if (URLS[k].indexOf("http") > -1) {
                        ...
                        urlCurrent = mainHost + '/images/' + affiliate_id + '/' + product_code + '/' + URLS[k]
                    }

                    var del = '<div onCLick="deletaImagen(\'secundaria\',\'' + affiliate_id + '\',\'' + product_code + '\',\'' + urlCurrent + '\',\'' + URLS[k] + '\', $(this))"  class="col-sm paiIcon2"><svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 21 21"  class="icoPopUpDelete superFica">&gt;<defs></defs><path class="a" d="M10.937,16H3.063A2.208,2.208,0,0,1,.875,13.778V2.666H.438a.444.444,0,0,1,0-.889H4.375V1.334A1.324,1.324,0,0,1,5.687,0H8.313A1.324,1.324,0,0,1,9.625,1.334v.444h3.937a.444.444,0,0,1,0,.889h-.437V13.778A2.208,2.208,0,0,1,10.937,16ZM1.75,2.666V13.778a1.325,1.325,0,0,0,1.313,1.334h7.875a1.325,1.325,0,0,0,1.313-1.334V2.666ZM5.687.889a.441.441,0,0,0-.437.445v.444h3.5V1.334A.441.441,0,0,0,8.313.889Zm3.5,11.556A.442.442,0,0,1,8.75,12V5.778a.437.437,0,1,1,.875,0V12A.441.441,0,0,1,9.188,12.445Zm-4.375,0A.441.441,0,0,1,4.375,12V5.778a.437.437,0,1,1,.875,0V12A.442.442,0,0,1,4.812,12.445Z" transform="translate(4 3)"></path></svg></div>'
                    if (pictureName != null) {
                        del = ''
                    }
            ...
            if (urlCurrent == '' || urlCurrent == null || urlCurrent == undefined) {
                if (URLS != undefined) {
                    if (URLS.indexOf("http") > -1) {
                        ...
                        urlCurrent = mainHost + '/images/' + affiliate_id + '/' + product_code + '/' + URLS
                        pictureName = URLS
                    }
                } else {
...
function deletaImagen(origem, affiliate_id, product_code, picture_url, picture_name, elemento) {

    //////console.log(origem)
    var alerta = bootbox.confirm({
        message: '<br> <h4 >Deseja deletar a imagem abaixo?</h4l><hr><img   src="' + picture_url + '" class=" notCrash" style=" width: 100%;  max-height: 20vh;  max-width: 20vh;  margin: auto;">',
        buttons: {
            confirm: {
                label: 'Sim',
                className: 'btn-light'
        ...
        callback: function (result) {
            if (result) {
                if (origem == 'principal') {
                    $(".firstImageShow").attr("src", 'http://localhost/images/default/produto-sem-imagem.jpg')
                    imagenPadrao(product_code, product_code, 'http://localhost/images/default/produto-sem-imagem.jpg')
                }
                $.ajax({
                    type: 'POST',
...
function getTagEspecial(texto, icone) {
    ...
    for (const k in tags) {
        if (tags[k] != '') {
            html += '<div style="width: 80px; padding-top: 5px" class="sTag">' +
                icone + ' <label  class="tagAmostra">' + tags[k] + '</label>' +
                "</div>";
        }
    }
    return html;
...
function getSimilarProducts(product_data, limite) {
    ...
    for (const k in product_data) {
        if (product_data[k] != undefined) {
            ...
            if (product_data[k].product_valor == null || product_data[k].product_valor == undefined || product_data[k].product_valor == '') {
                precoComum = product_data[k].product_valor
            }
            valorComDesconto = '<span class="preco2">R$ ' + Number(precoComum.toFixed(2)).toLocaleString() + '</span>'
            contador++
            var precoDescontado = precoComum


            if (contador <= limite) {
                if (product_data[k].product_site_discount_value != "" && product_data[k].product_site_discount_value != null) {
                    var descontos = JSON.parse(product_data[k].product_site_discount_value)
                    ...
                    if (descontos != null) {
                        ...
                        if (precoDescontado != null && precoDescontado != 'null') {
                            valorComDesconto = '<span class="preco2">R$ ' + precoDescontado + ' <span class="desconto2">R$ ' + Number(precoComum.toFixed(2)).toLocaleString() + '</span></span>';
                        }
                ...
                if (product_data[k].product_average_weight_value != "" && product_data[k].product_average_weight_value != null) {
                    pesoUnitario = '<span class="metricas">1 unidade - aproximadamente ' + product_data[k].product_average_weight_value + ' ' + product_data[k].product_average_weight_type + '</span>';
                } else {
                    pesoUnitario = '<span style="opacity: 0" class="metricas">1 unidade - aproximadamente ' + product_data[k].product_average_weight_value + ' ' + product_data[k].product_average_weight_type + '</span>';
                }
                var product_name = product_data[k].product_site_name
                if (product_data[k].product_site_name == undefined || product_data[k].product_site_name == null || product_data[k].product_site_name == "") {
                    product_name = product_data[k].product_descricao
                }

                html += '<div class="card" style="width: 200px; border-radius: 15px; margin-left: 10px">' +
                    ...
                    getTagEspecial(product_data[k].product_site_tags, '<i class="fas fa-heart"></i>') +
                    ...
                    '<li class="list-group-item  noBorder mainPicture listaThumb" style=" background: url(' + produtoURL(product_data[k].product_ean, PRODUCTS_IMAGES).DATA.thumbnail + '), url(https://api-smartcomerci.com.br/images/default/produto-sem-imagem.jpg);">' +
                    '<img   class="productImageThumb"  /> ' +
                    "</li>" +
                    '<li style="line-height: 0.2; height: 235px" class="list-group-item noBorder">' +
                    '<span class="fabricacao">' + product_data[k].product_fabricacao + '</span>' +
                    '<h3 class="tituloProdutoThumb">' + product_name + '</h3>' +
                    pesoUnitario +
                    '<br><span class="stars">' +
                    '<i class="fas fa-star"></i>' +
                    '<i class="fas fa-star"></i>' +
                    ...
                    '<label class="label" style="margin:auto; color: #f6b504 !important; font-size: 1.2rem;     display: inline-flex;">' + market2 + ' Adicionar </label>' +
                    "</li>" +
                    "</ul>" +
                    "</div>";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (const k in data) {
                ...
                var html = '<div product_code="' + data[k].product_code + '"  class="card PRDseleciona" style="width: 18%; border-radius: 15px; margin-left: 10px;   max-height: 200px">' +
                    '<ul class="list-group list-group-flush">' +
        ...
        case "relacionados":
            ...
            if (localStorage.relacionamentos == undefined) {
                ...
                if (relacionamentos.indexOf(tipoRelacionamento) > -1) {
                    ...
                    if (add == false) {
                        //////////console.log("passo 3 ")
                        relacionamentos = relacionamentos.replace("," + tipoRelacionamento, "")
...
function getProductCaract(product_code) {
    ...
    //////console.log(product_code)
    var MY_PRD = [], enviou = false, vouRetornar = []
    if (localStorage.PRODUCTS_SEARCH == undefined || localStorage.PRODUCTS_SEARCH == '') {
    ...
    if (MY_PRD.length > 0) {
        ...
        if (localStorage.resultSearch == undefined || localStorage.resultSearch == '') { } else {
...
function cardSelecionado(product_data) {
    ...
    if (product_data.product_site_name == null || product_data.product_site_name == undefined || product_data.product_site_name == "") {
        product_name = product_data.product_descricao
    }
    var html = '<div product_code="' + product_data.product_code + '" class="card PRDseleciona"   style="width: 18%; border-radius: 15px; margin-left: 10px; max-height: 200px">' +
        '<ul class="list-group list-group-flush">' +
        '<li class="list-group-item noBorder" style="border-bottom: none">' +
        iconGridMini2 +
        '<label class="numberCat">' + product_data.product_code + '</label>' +
        '<label product_code="' + product_data.product_code + '" product_code="' + product_data.product_code + '" onclick="removeThis2($(this))" product_code="' + product_data.product_code + '" remove="relacionados" style="float: right;margin-right: -10px;"  class="iconClose2"><i class="far fa-times-circle"></i></label>' +

        "</li>" +

        '<li class="list-group-item  noBorder mainPicture"  style="border-bottom: none">' +
        '<img   class="productImageThumb" src="' + produtoURL(product_data.product_ean, PRODUCTS_IMAGES).DATA.thumbnail + '" /> ' +
        "</li>" +
        '<li style="line-height: 0.2;    max-height: 70px;" class="list-group-item noBorder"  style="border-top: none">' +
        '<h3 class="tituloProdutoMini">' + product_name + '</h3>' +
        "</li>" +
        ...
        if (lista.length > 0) {
            for (const k in lista) {
                ...
                if (product_data != undefined) {
                    ...
                    html += '<div product_code="' + product_data.product_code + '" class="card PRDseleciona"   style="width: 18%; border-radius: 15px; margin-left: 10px; max-height: 200px">' +
                        '<ul class="list-group list-group-flush">' +
                        '<li class="list-group-item noBorder" style="border-bottom: none">' +
                        iconGridMini2 +
                        '<label class="numberCat">' + product_data.product_code + '</label>' +
                        '<label onclick="removeThis2($(this))" product_code="' + product_data.product_code + '" remove="relacionados" style="float: right;margin-right: -10px;"  class="iconClose2"><i class="far fa-times-circle"></i></label>' +
...
function cardSelecionadoPromo(product_data, onde) {
    var html = '<div  product_code="' + product_data.product_code + '" class="col-md-12 produtoParaPromocao">' +
        '<div style="margin: auto !important; border: 1px solid #EDF2F6; max-width: 80% !important"  class="input-group categoriaLabel2">' +
        '<div style="margin: auto; border: none" class="input-group  ico dropItems">' +
        '<div class="col-md-2 imgProcuraProduto" style="background: url(' + produtoURL(product_data.product_ean, PRODUCTS_IMAGES).DATA.thumbnail + '), url(https://api-smartcomerci.com.br/images/default/produto-sem-imagem.jpg)"></div>' +
        "</div>" +
...
function cardSelecionadoPromo2(lista, onde) {
    ...
    if (lista.length > 0) {
        for (const k in lista) {
            ...
            //////console.log(product_data)
            html += '<div product_code="' + product_data.product_code + '" class="col-md-12 produtoParaPromocao">' +
                '<div style="margin: auto !important; border: 1px solid #EDF2F6; max-width: 80% !important"  class="input-group categoriaLabel2">' +
                '<div style="margin: auto; border: none" class="input-group  ico dropItems">' +
                '<div class="col-md-2 imgProcuraProduto" style="background: url(' + produtoURL(product_data.product_ean, PRODUCTS_IMAGES).DATA.thumbnail + '), url(https://api-smartcomerci.com.br/images/default/produto-sem-imagem.jpg)"></div>' +
                "</div>" +
                ...
                product_data.product_descricao +
                "</label>" +
                '<label product_code="' + product_data.product_code + '"  onclick="removeProdutoDesconto($(this))"  class="iconClose"><i class="far fa-times-circle"></i></label>' +
                "</div>" +
                "</div><br>";
        }
...
function imagen_URL(EAN, product_code, PRODUCTS_IMAGES) {
    ...
    var img_UPLOAD = [],
        fullLista = []
    if (localStorage.LISTA_IMAGENS == undefined || localStorage.LISTA_IMAGENS == null) { } else {
    ...
    for (const k in img_UPLOAD) {
        if (URL_IMAGE.indexOf(PRODUCTS_IMAGES[k]) > -1) { } else {
                ...
                if (img_UPLOAD[k].indexOf("http") > -1) {
                    ...
                    if (contexto.indexOf("http") > -1) {
                        ...
                        if (contexto.indexOf("images/") > -1) {
                            ...
                            contexto = prefixo + 'images/' + localStorage.AFFILIATE_ID + '/' + product_code + '/' + contexto
...
function getIconsToShow(product_code) {
    ...
    var html = ''
    var imagens = imagen_URL(product_data.product_ean, product_data.product_code, PRODUCTS_IMAGES)
    //////////console.log("imagens")
    //////////console.log(imagens)
    //////////console.log("EAN")
    ...
    for (const k in imagens) {
        var contexto = ''
        if (imagens[k] != null && imagens[k] != '' && imagens[k] != undefined && imagens[k] != ' ') {
            contexto = imagens[k], prefixo = mainHost + '/'
            if (contexto.indexOf("http") > -1) { } else
                if (contexto.indexOf("produto-sem-imagem.jpg") > -1) {
                    contexto = 'images/default/produto-sem-imagem.jpg'
            ...
            html += '<div   class="iconShow">' +
                '<img   class="iconToShow" src="' + contexto + '">' +
                "</div><br>";
        }
...
function getS_TAGS(texto, icone) {
    ...
    if (lista != null) {
        ...
        for (const k in lista) {
            if (lista[k] != "") {
                html += '<div class="sTag">' +
                    icone + ' <label  class="tagAmostra">' + lista[k] + '</label>' +
                    "</div>";
            }
        }
    }
                ...
                success: function (picture) {
                    ...
                    try {
                        ...
                        $.ajax({
                            ...
                            success: function (data2) {
                                element.css("background", "url(" + mainHost + "/images/" + localStorage.AFFILIATE_ID + "/" + product_code + "/" + data2[0] + ")")
                            },
                            error: function (data2) {
                                try {
                                    if (data.responseJSON.message.indexOf("token") > -1) {
                    ...
                    $.ajax({
                        ...
                        error: function (data2) {
                            try {
                                ...
                                element.css("background", "url(images/default/produto-sem-imagem.jpg)")
                            }
    ...
    if (dados != undefined) {
        ...
        for (const k in data) {
            var html =
                ...
                '<div style="background: url(' + produtoURL(data[k].product_ean, PRODUCTS_IMAGES).DATA.thumbnail + '), url(https://api-smartcomerci.com.br/images/default/produto-sem-imagem.jpg)" class="bkgImagem"></div>' +
                '</div>' +
                ...
                '<label class="textoPreco11">' + data[k].product_valor + '</label>' +
                '</div>' +
                '<div  style="width: 170px; margin: auto;" class="col">' +
                '<div style=";padding-left: 5px !important;" myPicture="' + produtoURL(data[k].product_ean, PRODUCTS_IMAGES).DATA.thumbnail + '" product_value="' + data[k].product_valor + '" product_code="' + data[k].product_code + '" onclick="prdSeleciona5($(this))" class="prodSearchBtn PRDseleciona5">' +
            ...
            success: function (data) {
                ...
                for (const k in data) {
                    var html =
                        ...
                        '</div>' +
                        '<div  style="width: 170px; margin: auto" class="col">' +
                        '<div style=";padding-left: 5px !important;" myPicture="' + produtoURL(data[k].product_ean, PRODUCTS_IMAGES).DATA.thumbnail + '" product_code="' + data[k].product_code + '"  onclick="prdSeleciona5($(this))" class="prodSearchBtn PRDseleciona5">' +
                        '<label class="prodSearchTxt">Adicionar</label>' +
                        '</div>' +
                        '</div>' +
                        '</div>';
    ...
    if (dados != undefined) {
        ...
        for (const k in data) {
            var html =
                ...
                '</div>' +
                '<div  style="width: 170px; margin: auto;" class="col">' +
                '<div style=";padding-left: 5px !important;" myPicture="' + produtoURL(data[k].product_ean, PRODUCTS_IMAGES).DATA.thumbnail + '" product_code="' + data[k].product_code + '" onclick="addRelacionados($(this))" class="prodSearchBtn PRDseleciona4">' +
                '<label class="prodSearchTxt">Adicionar</label>' +
                '</div>' +
                '</div>' +
                '</div>';
    ...
    for (const k in data) {
        if (Number(data[k].product_code) == code) {
            var html = '<div product_code="' + data[k].product_code + '" class="card PRDseleciona"   style="width: 18%; border-radius: 15px; margin-left: 10px;   max-height: 200px">' +
                ...
                iconGridMini2 +
                '<label class="numberCat">' + data[k].product_code + '</label>' +
                '<label onclick="removeThis2($(this))" product_code="' + data[k].product_code + '" remove="relacionados" style="float: right;margin-right: -10px;"  class="iconClose2"><i class="far fa-times-circle"></i></label>' +
...
function setNutrition(elemento) {
    ...
    $(".listaNutritiva").each(function () {
        listaN.push({
            "titulo": $(this).find(".tt").val(), "quantidade": $(this).find(".qt").val()
        })
    })
...
function listaDevolveN(lista) {
    var html5 = ''
    for (const k in lista) {
        html5 += '<div style="padding:0 2%" class="row">' +
            '<div class="col-md-5">' +
            '<div style="padding:0 2%; margin-top: 2%" class="row">' +
            '<label style=" font-size: 20px;" class="label">Título</label><br> ' +
            "</div>" +
            ...
            '<div class="col-md-5">' +
            '<div style="padding:0 2%; margin-top: -2%" class="row">' +
            '<div class="group-input2"><input value="' + lista[k].titulo + '"  onchange="setNutrition($(this))" name="titulo" class=" tt form-control inputProduct nutriTitulo  nutritiones"   placeholder="Título" id="nomeProduto"></div><br> ' +
            "</div>" +
            "</div>" +
            '<div class="col-md-5">' +
            '<div style="padding:0 2%; margin-top: -2%" class="row">' +
            '<div class="group-input2"><input value="' + lista[k].quantidade + '"  onchange="setNutrition($(this))" name="valor" class=" qt form-control inputProduct nutriQuantidade  nutritiones"   placeholder="Quantidade" id="nomeProduto"></div><br> ' +
            "</div>" +
            "</div>" +
            '<div class="col-md-2">' +
            '<div onclick="limpaCampos($(this))" style="margin:auto" class="input-group iconOpaco ico etiquetaRedonda">' +
...
function criaNovaTag(elemento, e, elementoContainer) {
    ...
    if (code == 13) {
        var html = '<li class="list-item liVariacoes animate__animated ">' +
            '<label class="subSmart animate__animated animate__">' + elemento.val() + '<input  onchange="addTagMarcada($(this),\'' + elementoContainer + '\')" class="marcar2" type="checkbox"><span class="checkmark"></span></label>' +
            "</li>";
...
function addRelacionados(elemento) {
    if (localStorage.relacionados == null || localStorage.relacionados == 'null') { localStorage.relacionados = '[]' }
    var dados = JSON.parse(localStorage.resultSearch)
...
function prdSeleciona5(elemento) {
    ...
    if (localStorage.listaDescontos == null || localStorage.listaDescontos == 'null' || localStorage.listaDescontos == undefined) { localStorage.listaDescontos = JSON.stringify(listaDescontosOld) }
    var dados = JSON.parse(localStorage.resultSearch)
    //////console.log(dados)
    var prd_code = elemento.attr("product_code")
    var codigo_produto = Number(elemento.attr("product_code"))
    ...
    //////console.log(news)
    if (news)
        news.produtos.push(Number(prd_code))
...
async function buscaAvancadaFiltros() {
    ...
    cat.each(function () {
        LISTA_SEARCH.push({ "colmun": "categorias", "value": $(this).text(), "active": true })
    })
    tag.each(function () {
        LISTA_SEARCH.push({ "colmun": "tags", "value": $(this).text(), "active": true })
    })
    marc.each(function () {
        LISTA_SEARCH.push({ "colmun": "marcas", "value": $(this).text(), "active": true })
    })
...
function verFeedback() {
    ...
    for (const k in LISTA_SEARCH) {
        if (LISTA_SEARCH[k].colmun == "tags" || LISTA_SEARCH[k].colmun == "categorias" || LISTA_SEARCH[k].colmun == "marcas") {
...
function filter() {
    ...
    bootbox.alert({
        ...
        onShow: function () {
            ...
            $("#modalCentralFiltroInner").find(".modal-dialog").css("height", "50vh !important");
...
function listaAcoes(elemento) {
    ...
    if (listaMarcada.length > 0) {
        switch (elemento.val().trim().toLowerCase()) {
            case 'ativar': mostraAcoes("ATIVAR", listaMarcada.length); break;
            case 'desativar': mostraAcoes("DESATIVAR", listaMarcada.length); break;
            case 'excluir': mostraAcoes("EXCLUIR", listaMarcada.length); break;
...
function SIM() {
    ...
    var lista = []
    if (localStorage.listaSelecaoCheck != null && localStorage.listaSelecaoCheck != undefined) {
        try {
...
function orderBY(coluna, sentido) {
    ...
    personalRequest('productsOrderBy', lastID, 20, sentido, coluna, localStorage.AFFILIATE_ID, false)
}
...
async function buscaPeloFiltro(parametros, TOTAL_ITENS, continuando, limite, lastID, loading) {
...
function feedbackPeloFiltro(elemento, parametros, TOTAL_ITENS, continuando, limite) {
    var aguarde = '<div class="spinner-border text-light" style="color: white" role="status"><span class="sr-only">Loading...</span>  </div>'
    ...
    $.ajax({
        ...
        success: function (products) {
            ...
            if (quantos == undefined || quantos == 'undefined') { quantos = 0 }
            //////console.log(quantos)
