...
var CATEGORIES = [], MY_CATEGORIES = [], MINHAS_CATEGORIAS = [];
mostra()
function mostra() {
    $(".listaDeCategorias").html("");
    $.ajax({
        type: "POST",
        url: "https://api-smartcomerci.com.br:9090/getCatList",
        headers: {
        ...
        complete: function () {//console.clear()
        },
    });
}
            ...
            success: function (categories) {
                try {
                    ...
                    for (const k in myCategories) {
                        if (k == 8) {
                            $(".listaDeCategorias").append('<br><br><h1 class="hiperTitle">Outras Categorias <span class="hiperTitleSub">(Exibe em "todas as categorias" no menu)</h1><hr>')
                        }
                        $(".listaDeCategorias").append('<li class="itemSortable2">' + getCategorieShow(myCategories[k], (Number(k) + 1)) + '</li>')
                    }
                    ...
                    */
                    ...
                    $(".dropCategoriaContent").click(function () {
                        if ($(this).attr("dropado") == "nao" || $(this).attr("dropado") == undefined) {
                    ...
                    $(".ordena").dblclick(function () {
                        reorganiza(CATEGORIES_SHOW, $(this).attr("field"), $(this).attr("sentido"));
...
async function criaNovaCategoria() {
    ...
    $.ajax({
        type: "POST",
        url: "https://api-smartcomerci.com.br:9090/maisUmaCategoria",
        headers: {
            "x-access-token": localStorage.token,
        },
        data: {
...
function getCategorias(CATEGORIES) {
    var listaCategoriasPrimarias = [], CATEGORIAS_FULL = []
    ...
    for (const k in listaCategoriasPrimarias) {
        ...
        for (const s in CATEGORIES) {
            if (CATEGORIES[s].product_site_categories != null) {
                if (CATEGORIES[s].product_site_categories.split(",")[0] == listaCategoriasPrimarias[k].categoria) {
        ...
        CATEGORIAS_FULL.push({ "categoria": listaCategoriasPrimarias[k].categoria, "subCategorias": thisCategorieGroup, "cat_status": 0, "sub_status": '[]' })
...
function infoCategoria_keyWord(categoria) {
    ...
    for (const k in lis) {
        if (categoria == lis[k].affiliate_categorie_name) {
            if (lis[k].categorie_key_words != undefined && lis[k].categorie_key_words != "undefined" && lis[k].categorie_key_words != null) {
...
async function updateSequencia(mostra1) {
    ...
    $.ajax({
        ...
        success: function (resultado) {
            ...
            }else if(mostra1 == "reload"){ location.reload()} else {
                mostra();
            }
...
function getCategorieShow(dataCategorie, ID) {
    ...
    if (dataCategorie.subCategorias != "?") { totalSubCategorias = dataCategorie.subCategorias.split(",").length }
    var checkB = ''

    if (dataCategorie.affiliate_sub_categorie_status == 1) { checkB = 'checked="checked"' }
    if (dataCategorie.cat_status == 1) { checkB = 'checked="checked"' }
    var icone = '<img style="    max-width: 30px !important;" src="images/icons/frutas.png" />'
    if (dataCategorie.categorie_icon != null) {
        icone = '<img style="    max-width: 30px !important;" src="' + dataCategorie.categorie_icon + '" />'
    }

    var html = '<div title="\'' + dataCategorie.categorie_title + '\'"  description="\'' + dataCategorie.categorie_description + '\'"  draggable="true" class="categorie vouClonar">' +
        ...
        iconGridMini +
        '</div>' +
        '<div class="col-sm posicaoCategoria numberCat">' + ID + '</div>' +
        '</div>' +
        '</div>' +
        ...
        '</div>' +
        '<div class="col-sm"  >' +
        '<label  class="label nomeCategoria CATEGORIA_PRINCIPAL">' + dataCategorie.categoria + '</label>' +
        '</div>' +
        '<div class="col-sm">' +
        '<label class="label totalSubCategorias">' + (totalSubCategorias -1) + ' Subcategorias</label>' +
        '</div>' +
        '<div class="col-sm" style="opacity: 1;">' +
        '<div class="row">' +
        '<div class="switch__container ATIVA_CATEGORIA"   style="opacity: 1;margin-top: 5%;"><input ' + checkB + ' id="switch-shadow0310' + ID + '" class="switch switch--shadow ativa-me CHECK_PRINCIPAL" type="checkbox"><label for="switch-shadow0310' + ID + '"></label>' +
        '</div>' +
        ...
        '<div myCat="'+dataCategorie.categoria+'" onclick="outraSubCategoria($(this))"   style="    z-index: 999;;cursor:pointer;border-left: 5px dotted silver;margin-left: 7%;cursor: pointer; border-radius: 20px; font: bold 1rem Roboto; background-color: rgb(246, 181, 4); max-width: 200px; height: 40px; border: 2px solid rgb(246, 181, 4); float: left; opacity: 1;" class="input-group outraSubCat">' +
        ...
        '<ul id="sortable" class="listInner3 dropCategoria superSortable">' + getSubCategorias(dataCategorie.subCategorias, dataCategorie.affiliate_categorie_status, ID, dataCategorie.affiliate_categorie_name) +
...
function getSubCategorias(categoriasText, sub_status, ID, categoriaName) {
    ...
    for (const k in lista) {
            ...
            if (sub_status != undefined) {
                ...
                for (const c in confere) {
                    ...
                    if (confere[c].subCategoria.trim() == lista[k].trim()) {
                        if (confere[c].status == 1) { checkB = 'checked="checked"' }
                    }
                }
            }
        ...
        if (lista[k] != '' && lista[k].length > 0) {
            html += '<li  draggable="true" class="list-sub-item nova_sub newSub itemSortable" id="' + lista[k] + '_' + (Number(k) + 1) + '">' +
                ...
                '</svg>' +
                '</div>' +
                '<div class="col-sm  numberCat"><sp class="posicaoSubCategoria">' + (Number(k) + 1) + '</sp></div>' +
                '</div>' +
                '</div>' +
                ...
                */
                '<div class="col-sm"  >' +
                '<label id="'+(categoriaName?.replace(/ /g, '_') +'_'+lista[k]?.replace(/ /g, '_')+'_'+k)+'"  class="label nomeCategoria SUB_CATEGORIA">' + lista[k] + '</label>' +
                '</div>' +
                ...
                '<div class="col-sm" style="opacity: 1;">' +
                '<div class="row">' +
                '<div class="switch__container ATIVA_SUB_CATEGORIA"   style="opacity: 1;margin-top: 5%;"><input subCategoria="' + lista[k] + '" categoriaName="' + categoriaName + '" onchange="ativaSubCatInner($(this))" ' + checkB + ' id="switch-shadow01234' + k + ID + '9" class="switch switch--shadow ativa-me2" type="checkbox"><label for="switch-shadow01234' + k + ID + '9"></label>' +
                '</div>' +
                ...
                '<div ' +
                'title="\'' + atributoSubCat(categoriaName, lista[k], 'title') + '\'" ' +
                'description="\'' + atributoSubCat(categoriaName, lista[k], 'description') + '\'" ' +

                'smart="\'' + atributoSubCat(categoriaName, lista[k], 'smart') + '\'" ' +
                'maisVendidos="\'' + atributoSubCat(categoriaName, lista[k], 'maisVendidos') + '\'" ' +
                'ofertas="\'' + atributoSubCat(categoriaName, lista[k], 'ofertas') + '\'" ' +
                'personalizada="\'' + atributoSubCat(categoriaName, lista[k], 'personalizada') + '\'" ' +

                ' onClick="modalEditSubCategories(\'' + lista[k] + '\', \'' + categoriaName + '\',$(this))" class="input-group catEdit">' +
...
function modalEditCategories(element) {
    ...
    var dados = { "categoria": categoria.text(), "subCategorias": listaSubCategorias }
    ...
    var asWords = ""
    if (dadosCategoria.categorie_key_words != "" && dadosCategoria.categorie_key_words != undefined && dadosCategoria.categorie_key_words != null) {
    ...
    var html =
            ...
            <section class="areaBanner verticalScroll">
                ...
                <div class="descBanner8">
                    ...
                    <div class="btnQtdBanner">
                        <p class="txtQtdBanner listaBannersCatActive">${getBannerInnerMain(BANNERS?.banners, true).total}/${BANNERS?.banners?.length ? BANNERS?.banners?.length : 0}</p>
                    </div>
                </div>
                ...
                <div class="descBanner8">
                    Banners desativados
                    <div class="btnQtdBanner">
                        <p class="txtQtdBanner listaBannersCatInactive">${getBannerInnerMain(BANNERS?.banners, false).total}/${BANNERS?.banners?.length ? BANNERS?.banners?.length: 0 }</p>
                    </div>
                </div>
        ...
        '<div  style="margin-top: 3%;" class="row">' + iconesSmartCommerci(categoria.text()) +
        ...
        '<div  style="margin-top: 3%;" class="row iconesClientes">' + iconesSmartCommerci2(categoria.text()) +
        ...
        '<div class="group-input2"  style="background: #F0F0F0 0% 0% no-repeat padding-box;border: 1px solid #EFEFEF;border-radius: 5px;"><input  style="background: none" class="form-control inputProduct"  fieldName="affiliate_categorie_name"  onchange="fila($(this), \'' + categoria.text() + '\',\'updateCategoriaDetalhe\')" placeholder="Produtos relacionados" id="' + aleatoryID() + '" value="' + categoria.text() + '"></div><br> ' +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div class="col-md-12">' +
        '<div class="group-input2"  style="background: #F0F0F0 0% 0% no-repeat padding-box;border: 1px solid #EFEFEF;border-radius: 5px;"><input fieldName="categorie_title" onchange="fila($(this), \'' + categoria.text() + '\', \'updateCategoriaDetalhe\')" value="' + dadosCategoria.categorie_title + '" style="background: none" class="form-control inputProduct" placeholder="No Kalimera você encontra tudo em frutas"  id="' + aleatoryID() + '" ></div><br> ' +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div class="col-md-12">' +
        '<div class="group-input2"  style="padding: 1%;background: #F0F0F0 0% 0% no-repeat padding-box;border: 1px solid #EFEFEF;border-radius: 5px;"><textarea fieldName="categorie_description"  id="' + aleatoryID() + '" onchange="fila($(this), \'' + categoria.text() + '\',\'updateCategoriaDetalhe\')" value="' + dadosCategoria.categorie_description + '" placeholder="Frutas no Kalimera. Compre online, limão, tangerina, kiwi e muitas outras frutas com os melhores preços e fretegratis."  style="background: #EFEFEF; border:none; font-size: 1.3rem; max-height: 100%" rows="3"   class="form-control">' + dadosCategoria.categorie_description + '</textarea>' +
        "</div>" +
        "</div>" +
        "</div><br><br>" +
        ...
        '<div><input categorie_name="' + categoria.text() + '" type="text" fieldName="categorie_key_words" container="listaPalavrasKey" onkeydown="addWordKey($(this), this)" class="form-control entraPalavra" placeholder="Digite sua palavra aqui e pressione enter..." style="border: none; font-size: 1.3rem;height: auto; width: 90%;"/></div>' +
        asWords +
        "</div>" +
        "</div>" +
        "</div>" +
...
function modalEditSubCategories(subCategoria, categoria, element) {
    ...
    var dataSubCategoria = [], todasCategorias = []
    if (localStorage.MINHAS_CATEGORIAS != undefined && localStorage.MINHAS_CATEGORIAS != null && localStorage.MINHAS_CATEGORIAS != 'null' && localStorage.MINHAS_CATEGORIAS != '') {
   ...
   console.log("data sub",dataSubCategoria)
    ...
    try{
        status = JSON.parse(ajustStrigfy(dataSubCategoria.affiliate_categorie_status))
    }catch(e){}
    ...
    var smart = element.attr("smart"), ofertas = element.attr("ofertas"), title = element.attr("title"), description = element.attr("description"), key_words = element.attr("key_words"), maisVendidos = element.attr("maisVendidos"), personalizada = element.attr("personalizada");
    if (element.attr("title") == 'undefined') { title = '' }
    if (element.attr("smart") == 'undefined') { smart = 0 }
    if (element.attr("description") == 'undefined') { description = '' }
    if (element.attr("key_words") == 'undefined') { key_words = '' }
    if (element.attr("maisVendidos") == 'undefined') { maisVendidos = 0 }
    if (element.attr("ofertas") == 'undefined') { ofertas = 0 }
    if (element.attr("personalizada") == 'undefined') { personalizada = 0 }

    if (Number(ofertas) == 1) { ofertas = ' checked="true"' }
    if (Number(personalizada) == 1) { personalizada = ' checked="true"' }
    if (Number(smart) == 1) { smart = ' checked="true"' }
    if (Number(maisVendidos) == 1) { maisVendidos = ' checked="true"' }
    ...
    if (essaSubCat.key_words == undefined || essaSubCat.key_words == null || essaSubCat.key_words == '') {
        key_words = []
    } else {
    ...
    var html =
        ...
        <input onchange="uploadBannerCatVertical($(this))" type="file" id="pegaBannerCatVertical" style="display:none">
            <section class="areaBanner verticalScroll">
                ...
                <div class="descBanner8">
                    Banners ativos
                    <div class="btnQtdBanner">
                        <p class="txtQtdBanner listaBannersCatActive">${getBannerInner(essaSubCat?.banners, true).total}/${essaSubCat?.banners?.length ? essaSubCat?.banners?.length : 0}</p>
                    </div>
                </div>
                <ul id="listaBannersCatActive" style="list-style: none;" class=" superSortable fullSortable ui-sortable">
                ...
                <div class="descBanner8">
                    Banners desativados
                    <div class="btnQtdBanner">
                        <p class="txtQtdBanner listaBannersCatInactive">${getBannerInner(essaSubCat?.banners, false).total}/${essaSubCat?.banners?.length ?essaSubCat?.banners?.length: 0 }</p>
                    </div>
                </div>
                <ul id="listaBannersCatInactive" style="list-style: none;" class=" superSortable fullSortable ui-sortable">
        ...
        '<div style="margin: 1% 2%;" class="switch__container"><input fieldName="smart" subCategorieName="\'' + subCategoria + '\'" onchange="setSubCatAtt($(this))" ' + smart + ' id="switch-shadow1988" class="switch switch--shadow" type="checkbox" /><label for="switch-shadow1988"></label></div>' +
        ...
        '<div style="margin-top: 3% !important;" class="col-md-12 grupo">' +
        '<div style="padding: 0 2%; margin-top: 2%;" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container"><input fieldName="maisVendidos"  subCategorieName="\'' + subCategoria + '\'" onchange="setSubCatAtt($(this))" ' + maisVendidos + ' id="switch-shadow1999" class="switch switch--shadow" type="checkbox" /><label for="switch-shadow1999"></label></div>' +
        ...
        '<div style="margin-top: 3% !important;" class="col-md-12 grupo">' +
        '<div style="padding: 0 2%; margin-top: 2%;" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container"><input fieldName="ofertas" subCategorieName="\'' + subCategoria + '\'" onchange="setSubCatAtt($(this))" ' + ofertas + ' id="switch-shadow1900" class="switch switch--shadow" type="checkbox" /><label for="switch-shadow1900"></label></div>' +
        ...
        '<div style="margin-top: 3% !important;" class="col-md-12 grupo">' +
        '<div style="padding: 0 2%; margin-top: 2%;" class="row">' +
        '<div style="margin: 1% 2%;" class="switch__container"><input fieldName="personalizada" subCategorieName="\'' + subCategoria + '\'" onchange="setSubCatAtt($(this))" ' + personalizada + ' id="switch-shadow1944" class="switch switch--shadow" type="checkbox" /><label for="switch-shadow1944"></label></div>' +
        ...
        '<input  ' + activeOrNot + '   fieldName="status"  subCategorieName="\'' + subCategoria + '\'" onchange="setSubCatAtt($(this))" id="switch-shadow18" class="switch switch--shadow" type="checkbox" />' +
        '<label for="switch-shadow18"></label>' +
        "</div>" +
        ...
        '<div class="group-input2"  style="background: #F0F0F0 0% 0% no-repeat padding-box;border: 1px solid #EFEFEF;border-radius: 5px;"><input  style="background: none" class="form-control inputProduct" placeholder="Produtos relacionados" id="nomeCategoria" idParent="'+textElement.attr("id")+'"  fieldName="subCategoria" subCategorieName="\'' + subCategoria + '\'" onchange="setSubCatAtt($(this))" value="' + subCategoria + '"></div><br> ' +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div class="col-md-12">' +
        '<div class="group-input2"  style="background: #F0F0F0 0% 0% no-repeat padding-box;border: 1px solid #EFEFEF;border-radius: 5px;"><input value="' + title + '"   fieldName="title" subCategorieName="\'' + subCategoria + '\'" onchange="setSubCatAtt($(this))"  style="background: none" class="form-control inputProduct" placeholder="No Kalimera você encontra tudo em frutas" id="' + aleatoryID() + '" ></div><br> ' +
        ...
        '<div style="padding:0 2%" class="row">' +
        '<div class="col-md-12">' +
        '<div class="group-input2"  style="padding: 1%;background: #F0F0F0 0% 0% no-repeat padding-box;border: 1px solid #EFEFEF;border-radius: 5px;"><textarea  fieldName="description" subCategorieName="\'' + subCategoria + '\'" onchange="setSubCatAtt($(this))" placeholder="Frutas no Kalimera. Compre online, limão, tangerina, kiwi e muitas outras frutas com os melhores preços e fretegratis."  style="background: #EFEFEF; border:none; font-size: 1.3rem; max-height: 100%" rows="3"   class="form-control">' + description + '</textarea>' +
        "</div>" +
        "</div>" +
        "</div><br><br>" +
        ...
        '<div class="col-md-12">' +
        '<div class="listaPalavrasKey  notScroll verticalScroll" contentEditable="true" placeholder="digite aqui e aperte enter..." class="group-input2"  style="padding: 1%;background: #F0F0F0 0% 0% no-repeat padding-box;border: 1px solid #EFEFEF;border-radius: 5px; min-height: 150px !important;">' +
        '<div><input categorie_name="' + subCategoria + '" container="listaPalavrasKey"  fieldName="key_words" onkeydown="addWordKey2($(this),this)" type="text" class="form-control entraPalavra" placeholder="Digite sua palavra aqui e pressione enter..." style="border: none; font-size: 1.3rem;height: auto; width: 90%;"/></div>' +
        WordKeys2(key_words) +
        "</div>" +
        "</div>" +
        "</div>" +
...
function iconesSmartCommerci(categorieName) {
    var cate = JSON.parse(localStorage.MINHAS_CATEGORIAS), essaCat = []
    for (const k in cate) {
    ...
    var html = ''
    var ver = essaCat.categorie_icon?.split('/') ? essaCat.categorie_icon.split('/') : []
    ver = ver[ver.length - 1]?.replace(/ /g, '')
    function sugestao(n) {
        switch (n) {
            case 28: return 'acougue';            case 3: return 'acougue';            case 5: return 'acougue';            case 6: return 'acougue';            case 10: return 'frutas';            case 11: return 'frutas';            case 23: return 'frutas';            case 9: return 'padaria';            case 11: return 'padaria';            default: return 'outros';        }
    }
...
function addWordKey(elemento, e) {
    ...
    if (code == 13) {
        $("." + elemento.attr("container")).append('<div class="input-group categoriaLabel categoriasPalavras"><label class="aPalavra">' + elemento.val() + '</label><label onclick="meRemoveX($(this))" class="iconClose"><i class="far fa-times-circle"></i></label></div>')
...
function addWordKey2(elemento, e) {
    ...
    if (code == 13) {
        $("." + elemento.attr("container")).append('<div class="input-group categoriaLabel categoriasPalavras"><label class="aPalavra">' + elemento.val() + '</label><label onclick="meRemoveX2($(this))" class="iconClose"><i class="far fa-times-circle"></i></label></div>')
        palavrasKey(elemento.val(), "add")
        elemento.val("")
...
function WordKeys(text) {
    ...
    for (const k in text) {
        if (text[k] != "") {
            html += '<div class="input-group categoriaLabel categoriasPalavras"><label class="aPalavra">' + text[k] + '</label><label onclick="meRemoveX($(this))" class="iconClose"><i class="far fa-times-circle"></i></label></div>'
        }
...
function WordKeys2(text) {
    ...
    for (const k in text) {
        if (text[k] != "") {
            html += '<div class="input-group categoriaLabel categoriasPalavras"><label class="aPalavra">' + text[k] + '</label><label onclick="meRemoveX2($(this))" class="iconClose"><i class="far fa-times-circle"></i></label></div>'
        }
    }
    return html
}
...
function palavrasKey(palavra, regra) {
    var palavras = []
    if (localStorage.PALAVRAS_KEY != "undefined" && localStorage.PALAVRAS_KEY != undefined && localStorage.PALAVRAS_KEY != null && localStorage.PALAVRAS_KEY != "" && localStorage.PALAVRAS_KEY != "null") {
...
function fila(elemento, categorieName, funcao) {
    ...
    if (sessionStorage.NEED_TO_SAVE == undefined || sessionStorage.NEED_TO_SAVE == null || sessionStorage.NEED_TO_SAVE == "") {
        var lista_mudancas = []
        lista_mudancas.push({
            "ID_ELEMENTO": elemento.attr("id"), "NOME_CATEGORIA": categorieName, "FUNCAO": funcao
        })
    ...
    if (sessionStorage.NEED_TO_SAVE != undefined || sessionStorage.NEED_TO_SAVE != null || sessionStorage.NEED_TO_SAVE == "") {
        ...
        if (mudancas != undefined) {
            if (mudancas.length == 0) {
                ...
                bootbox.confirm({
                    message: "<p style='font: normal normal normal 24px Roboto;'>Deseja <b>CANCELAR</b> as alterações feitas?</p>",
                    onShow: function () {
...
function atributoSubCat(categoria, subCat, fieldName) {
    ...
    var lista = JSON.parse(localStorage.MINHAS_CATEGORIAS), listaSub = []
    for (const k in lista) {
    ...
    for (const k in listaSub) {
        if (subCat == listaSub[k].subCategoria) {
            switch (fieldName) {
                case 'title': return listaSub[k].title;
                case 'description': return listaSub[k].description;
                case 'key_words': return listaSub[k].key_words;
                case 'smart': return listaSub[k].smart;
                case 'maisVendidos': return listaSub[k].maisVendidos;
                case 'ofertas': return listaSub[k].ofertas;
                case 'personalizada': return listaSub[k].personalizada;


                default: return listaSub[k];
            }
...
function setSubCatAtt(elemento) {
    ...
    let changeName = false, lastName = subCategorieName

    switch (fieldName) {
        case 'subCategoria': listaSub.subCategoria = dado; localStorage.SUB_CAT_ATUAL = dado; changeName = true
            let lista = JSON.parse(localStorage.MINHAS_CATEGORIAS)
            for (const k in lista) {
                if (lista[k].categoria == localStorage.CAT_SUB_EDIT) {
                    let names = lista[k].affiliate_sub_categorie_name?.replace(`${lastName}`,`${dado}`)
                    let names2 = lista[k].subCategorias?.replace(`${lastName}`,`${dado}`)
            ...
            break;
        case 'status': if (dado) { dado = 1 } else { dado = 0 }
            listaSub.status = dado;
            break;
        case 'title': listaSub.title = dado;
            break;
        case 'description': listaSub.description = dado;
            break;
        case 'key_words': var ele = localStorage.PALAVRAS_KEY; listaSub.key_words = ele;
            break;
        case 'smart': listaSub.smart = dado;
            break;
        case 'maisVendidos': listaSub.maisVendidos = dado;
            break;
        case 'ofertas': listaSub.ofertas = dado;
            break;
        case 'personalizada': listaSub.personalizada = dado;
            break;
        case 'banners': listaSub.personalizada = dado;
            break;
...
function salvaModalSubCategoria() {
    ...
    if (localStorage.CAT_SUB_EDIT != undefined && localStorage.CAT_SUB_EDIT != '') {
        faz = true
    }
...
function ativaSubCatInner(elemento) {
    var lista = JSON.parse(localStorage.MINHAS_CATEGORIAS), todasSub = [], essaCat = []
            ...
            <li class="itemSortable2 ui-sortable-handle"> 
                <div class="areaBannerInner">
                    <div class="borderLeft">
                        ...
                        <br />
                        <div style="text-align: center" class="switch__container"><input onchange="changeMyActive($(this))" thisUrl="${imgURLs[k].url}" ${imgURLs[k].active == true ? 'checked="true"' : ''} id="switch-shadow1778${RANDOM}"
                                class="switch switch--shadow" type="checkbox" /><label style="    margin-top: 15px;"
                                for="switch-shadow1778${RANDOM}"></label></div>
                        <div  onclick="removeBanner('${imgURLs[k].url}','banners', $(this))" style="margin-top: 15px" class=" deleteThis">
                    ...
                    <div  alvo="update" onclick="alteraBannerCat($(this))" ${imgURLs[k].url ? `style="background-size: cover !important; background-position: center; zoom: 100%;background: url(${imgURLs[k].url})"`: ''}  class="borderRight">
                    </div>
                </div>
                ...
                <li class="itemSortable2 ui-sortable-handle"> 
                    <div class="areaBannerInner">
                        <div class="borderLeft">
                            ...
                            <br />
                            <div style="text-align: center" class="switch__container"><input onchange="changeMyActiveMain($(this))" thisUrl="${imgURLs[k].url}" ${imgURLs[k].active == true ? 'checked="true"' : ''} id="switch-shadow1778${RANDOM}"
                                    class="switch switch--shadow" type="checkbox" /><label style="    margin-top: 15px;"
                                    for="switch-shadow1778${RANDOM}"></label></div>
                            <div  onclick="removeBannerMain('${imgURLs[k].url}','banners', $(this))" style="margin-top: 15px" class=" deleteThis">
                                <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 21 21"
                                    style="fill: #f6b504;margin: 9px;">
                                    &gt;
                                    <defs></defs>
                        ...
                        <div  alvo="update" onclick="alteraBannerCatMain($(this))" ${imgURLs[k].url ? `style="background-size: cover !important; background-position: center; zoom: 100%;background: url(${imgURLs[k].url})"`: ''}  class="borderRight">
                        </div>
                    </div>
                    <button onclick="modalAlteraLinks($(this))" class="botaoParaLink">Alterar Link</button>
                </li>`
        ...
        let LENGTH = $(".bnnVertical").length ? $(".bnnVertical").length + 1 : 1
            ...
            <div class="areaBannerInner bnnVertical" style=" margin-top: 15px;min-height: 490px !important">
                <div class="borderLeft" style="    min-height: 490px !important">
                    ...
                    <br />
                    <div style="text-align: center" class="switch__container"><input thisUrl="${imgURLs[k].url}" onchange="changeMyActiveVertical($(this))" id="switch-shadow1799${RANDOM}" ${imgURLs[k].active == true ? 'checked="true"' : ''}
                            class="switch switch--shadow" type="checkbox" /><label style="    margin-top: 15px;"
                            for="switch-shadow1799${RANDOM}"></label></div>
                    <div onclick="removeBanner('${imgURLs[k].url}','bannersVertical', $(this))" style="margin-top: 15px" class=" deleteThis">
                        <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 21 21"
                            style="fill: #f6b504;margin: 9px;">
                            &gt;
                            <defs></defs>
                ...
                <div alvo="update" onclick="alteraBannerCatVertical($(this))" ${imgURLs[k].url ? `style="background-repeat: no-repeat;background-repeat: no-repeat !important; background-position: center; zoom: 80%; min-height: 490px; width: calc(100% - 100px); float: right; background: url(${imgURLs[k].url})"`: ''}   class="borderRightVertical">
                </div>
                <button onclick="modalAlteraLinks($(this))" class="botaoParaLink">Alterar Link</button>
            </div>`
            ...
            let LENGTH = $(".bnnVertical").length ? $(".bnnVertical").length + 1 : 1
                ...
                <div class="areaBannerInner bnnVertical" style=" margin-top: 15px;min-height: 490px !important">
                    <div class="borderLeft" style="    min-height: 490px !important">
                        ...
                        <br />
                        <div style="text-align: center" class="switch__container"><input thisUrl="${imgURLs[k].url}" onchange="changeMyActiveVerticalMain($(this))" id="switch-shadow1799${RANDOM}" ${imgURLs[k].active == true ? 'checked="true"' : ''}
                                class="switch switch--shadow" type="checkbox" /><label style="    margin-top: 15px;"
                                for="switch-shadow1799${RANDOM}"></label></div>
                        <div onclick="removeBannerMain('${imgURLs[k].url}','bannersVertical', $(this))" style="margin-top: 15px" class=" deleteThis">
                            <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 21 21"
                                style="fill: #f6b504;margin: 9px;">
                                &gt;
                                <defs></defs>
                    ...
                    <div alvo="update" onclick="alteraBannerCatVerticalMain($(this))" ${imgURLs[k].url ? `style="background-repeat: no-repeat;background-repeat: no-repeat !important; background-position: center; zoom: 80%; min-height: 490px; width: calc(100% - 100px); float: right; background: url(${imgURLs[k].url})"`: ''}   class="borderRightVertical">
                    </div>
                    <button onclick="modalAlteraLinks($(this))" class="botaoParaLink">Alterar Link</button>
                </div>`
      ...
      $.ajax({
        ...
        success: function (data) {
            ...
            if($("#"+element.attr("target")).attr("alvo") == 'novo'){
                ...
                elementoNew.append(
                    getBannerInner([
                        {"active":false, url:data.path?.replace('./public','https://api-smartcomerci.com.br:9090') }
                    ], false).html
                )
                ...
                if(subs[k].subCategoria.trim() == thisCategory.trim()){
                    ...
                    try{
                        if(bnn[0].url == undefined){
                            subs[k]['banners'] = [{active:element.parent().parent().find('input')[0].checked, url: `${data.path?.replace('./public','https://api-smartcomerci.com.br:9090')}`}]
                        }else{
            ...
            for(const k in subs){
                if(subs[k].categoria.trim() == thisCategory.trim()){
                    if(subs[k]['subcategorie_banners'] == null){
                        ...
                        try{
                            if(subs[k]['subcategorie_banners'].banners){ }else{
                                subs[k]['subcategorie_banners'] = JSON.parse(subs[k]['subcategorie_banners'])
                            }
        ...
        success: function (data) {
            ...
            for(const k in subs){
                if(subs[k].subCategoria.trim() == thisCategory.trim()){
                    ...
                    try{
                        if(bnn[0].url == undefined){
                            subs[k]['bannersVertical'] = [{active:element.parent().parent().find('input')[0].checked, url: `${data.path?.replace('./public','https://api-smartcomerci.com.br:9090')}`}]
                        }else{
                            bnn.push({active:element.parent().parent().find('input')[0].checked, url: `${data.path?.replace('./public','https://api-smartcomerci.com.br:9090')}`})
...
function changeMyActive(element){
    ...
    if(element[0].checked){ 
        ...
        $(".listaBannersCatActive").html(
            $("#listaBannersCatActive").find('li').length +"/"+me.parent().parent().find('.itemSortable2').length
        )
        up()
        ...
        $(".listaBannersCatInactive").html(
            $("#listaBannersCatInactive").find('li').length +"/"+me.parent().parent().find('.itemSortable2').length
        )
        up()
        ...
        for(const k in subs){
            if(subs[k].categoria.trim() == thisCategory.trim()){
                if(subs[k]['subcategorie_banners'] == null){
                    ...
                    try{
                        if(subs[k]['subcategorie_banners'].bannersVertical){ }else{
                            subs[k]['subcategorie_banners'] = JSON.parse(subs[k]['subcategorie_banners'])
                        }
    ...
    for(const k in subs){
        if(subs[k].categoria.trim() == thisCategory.trim()){
            if(subs[k]['subcategorie_banners'] == null){
                ...
                try{
                    if(subs[k]['subcategorie_banners'][table]){ }else{
                        subs[k]['subcategorie_banners'] = JSON.parse(ajustStrigfy(subs[k]['subcategorie_banners']))
                    }
