...
$(".continuar").click(function(){
    ...
    if(!Number(localStorage.idPageStartup)){number = 1}
    switch(localStorage.stage){      
        case "1":    
          
            var anomaly = [], codeVerify = ""
        ...
        case "3":
            var anomaly = [], dadosEndereco = []
            $(".pegaInput").each(function () {
                dadosEndereco.push({"tag":$(this).attr("placeholder"), "value":$(this).val()})
                if($(this).val() == ""){
                    anomaly.push($(this))
                }
            })
        ...
        case "4":
            ...
            break
        case "5":
            if(localStorage.segmentoPrincipal == undefined || localStorage.segmentoPrincipal == "undefined" || localStorage.segmentoPrincipal == ""){
...
$(".addPessoasClick").click(function() {   
    ...
    '<div class="row" style="max-width: 80%;margin: auto;">'+
        '<div class="subCardModal">'+
            ...
            '<div class="row"><div style="max-width: 15%">'+checkVerde2+'</div> <p class="itemEscolhido">Visão global da loja</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkVerde2+'</div> <p class="itemEscolhido">Pedidos</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkVerde2+'</div> <p class="itemEscolhido">Marketing</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkVerde2+'</div> <p class="itemEscolhido">Novos usuários da loja</p></div>'+               
        '</div>'+
        '<div class="subCardModal">'+
            '<h4 class="txtSubCard">Marketing</h4>'+
            '<div class="row"><div style="max-width: 15%">'+checkSilver+'</div> <p class="itemEscolhido">Visão global da loja</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkVerde2+'</div> <p class="itemEscolhido">Pedidos</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkVerde2+'</div> <p class="itemEscolhido">Marketing</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkSilver+'</div> <p class="itemEscolhido">Novos usuários da loja</p></div>'+               
        ...
        '<div class="subCardModal">'+
            ...
            '<div class="row"><div style="max-width: 15%">'+checkSilver+'</div> <p class="itemEscolhido">Visão global da loja</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkVerde2+'</div> <p class="itemEscolhido">Pedidos</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkSilver+'</div> <p class="itemEscolhido">Marketing</p></div>'+
            '<div class="row"><div style="max-width: 15%">'+checkSilver+'</div> <p class="itemEscolhido">Novos usuários da loja</p></div>'+               
    ...
    '<div class="areaMail verticalScroll notScroll">'+
        '<h5 class="subSub2">Insira o e-mail e aperte "ENTER": </h5>'+mailIcon+
        '<div class="mails row">'+
            ...
            if(pacote.length == 0){
               //alert("Nenhum pacote de acessos foi selecionado!")
                ...
                if(peoples.length == 0){
                   ...
                   atualA = JSON.parse((localStorage.ACESSOS ? localStorage.ACESSOS : '[]'))
...
$(".btnPlusAddLoja").click(function () {
    var html =      '<div categoria="afiliada" class="cardLoja">'+
...
$(".enviaCodigo").click(function() {
    if($("#emailMaster").val() != "" && $("#emailMaster").val().indexOf("@") >-1){     
...
function addAccess(elemento,tipo, lista) {
    for(const k in lista){
        var html = '<div class="funcionario">'+
                        '<div class="row">'+
                            '<div class="iconFuncionario">'+lista[k].split("")[0].toUpperCase()+'</div>'+
                            '<p class="contatoText">'+lista[k]+'</p>'+
                            '<p class="txtFuncionario">perfil '+tipo+'</p>'+
                            '<div mail="'+lista[k]+'" onclick="editaContato1($(this),\''+lista[k]+'\')" style="max-width: 5%; float:right; cursor:pointer"><svg xmlns="http://www.w3.org/2000/svg" style="margin:6% auto; fill: #50C682" width="20" height="20" viewBox="0 0 20 20"><defs></defs><path class="a" d="M12.929,15H2.083A2.086,2.086,0,0,1,0,12.917V2.084A2.086,2.086,0,0,1,2.083,0H7.928a.417.417,0,0,1,0,.833H2.083a1.252,1.252,0,0,0-1.25,1.25V12.917a1.252,1.252,0,0,0,1.25,1.25H12.929a1.251,1.251,0,0,0,1.25-1.25V7.084a.417.417,0,0,1,.834,0v5.833A2.086,2.086,0,0,1,12.929,15Zm-5-5.833H6.262a.417.417,0,0,1-.417-.416V7.084a.417.417,0,0,1,.122-.295L12.633.122a.417.417,0,0,1,.59,0L14.89,1.789a.416.416,0,0,1,0,.59L8.223,9.046A.414.414,0,0,1,7.928,9.167Zm5-8.161h0l-6.25,6.25V8.334H7.756l6.25-6.25L12.929,1.007Z" transform="translate(2.488 2.499)"></path></svg></div>'+
...
function editaContato1(element,contato) {
    ...
    var fieldText = element.parent().parent().parent().parent().find(".fieldText"), listaPessoal = element.parent().parent().parent().parent().find(".listaPessoal")
       ...
       element.parent().parent().remove()
        ...
        '<div class="areaMail verticalScroll notScroll">'+
            '<h5 class="subSub2">Insira o e-mail e aperte "ENTER": </h5>'+mailIcon+
            '<div class="mails row">'+
            '<span class="mail">'+contato+' &nbsp;  <img class="closeBtn" src="images/icons/cancel.png" style="width: 15px; height: 15px;margin: auto -5px auto auto;cursor:pointer"></span>'+
            '<input onchange="validacaoEmail($(this))" class="vaiEmail" value="'+contato+'" placeholder="email@mail.com"/>'+
            ...
            for(const k in ACESSOS){
                ...
                for(const a in mails){
                    $("#listaDeAcessosConcedidos").append(
                        '<tr>'+
                            '<td>'+mails[a]+'</td>'+
                            '<td>'+ACESSOS[k].type[0]+'</td>'+
                            '<td>'+ACESSOS[k].onde+'</td>'+
